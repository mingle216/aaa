...
export const getI18nSettings = () => {
  return {
    ...
    monthNamesShort: months.map((index) => window.shell.getLanguageValue("CUS_CARD_GXCAL", `monthShort${index}`, `${ index + 1 }月`)),
    monthNames: months.map((index) => window.shell.getLanguageValue("CUS_CARD_GXCAL", `month${index}`, `${ index + 1 }月`)),
    amPm: ['am', 'pm']
  };
};

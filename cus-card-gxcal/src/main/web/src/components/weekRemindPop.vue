    ...
    <ContainerScroll :barKeepShow="true">
      <div class="weekRemindPop">
        <span style="margin:5px 3px 0 0">{{
          $Lan("CUS_CARD_GXCAL", "weekRemindEnabled", "是否开启")
        }}</span>
        <we-switch
        ...
        <div>
          <p class="requiredTitle">
            {{ $Lan("CUS_CARD_GXCAL", "remindTime", "提醒日期") }}
          </p>
          <we-select
            ...
            :placeholder="
              $Lan('CUS_CARD_GXCAL', 'selectPlaceholder', '请选择')
            "
            clearable
          ...
          >
            <we-option
              ...
              :key="item.value"
              :label="$Lan('CUS_CARD_GXCAL', item.label, item.labelZh)"
              :value="item.value"
            >
            </we-option>
          </we-select>
          <div
            ...
            class="errorMsg"
          >
            {{ $Lan("CUS_CARD_GXCAL", "errorMsg", "提醒时间不为空") }}
          </div>
          <p style="margin:5px 0;">
            {{ $Lan("CUS_CARD_GXCAL", "remindTimeHour", "提醒时间") }}
          </p>
          <we-select
          ...
          <p style="margin: 5px 0">
            {{ $Lan("CUS_CARD_GXCAL", "remindWay", "提醒方式") }}
          </p>
          <we-checkbox-group
        ...
        <div class="btn-list">
          <we-button size="mini" @click="handleCancel()">{{
            $Lan("CUS_CARD_GXCAL", "cancel", "取消")
          }}</we-button>
          <we-button size="mini" type="primary" @click="handleSave()">{{
            $Lan("CUS_CARD_GXCAL", "confirm", "确定")
          }}</we-button>
        </div>
      </div>
    </ContainerScroll>
        ...
        (res) => {
          if (res && res.errcode === "0") {
            this.$message.success({
              message: this.$Lan("CUS_CARD_GXCAL", "successMsg", "修改成功"),
            });
            //表单数据初始化
            this.$emit("cancelPop");
            this.$emit("update");

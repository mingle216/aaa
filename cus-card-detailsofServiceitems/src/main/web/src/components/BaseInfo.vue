<template>
  ...
  <div class="basicInformation" style="position: relative">
    ...
    <ul class="dzgrxx">
      <li v-for="(item,index) in tableData" :key="index" :style="item.fieldName=='事项名称'? 'width:100%' : ''"><div><p class="portal-font-color-lv1">{{item.fieldName}}</p><p class="portal-font-color-lv1">{{item.fieldValue}}</p></div></li>
    </ul>
  ...
  computed: {
    tableData() {
      if (this.serviceItemInfo) {
        console.log(this.serviceItemInfo.baseInfos)
...
.dzgrxx>li>div>p:first-child{
  background: rgb(246, 246, 248);
  width: 120px;
}
.dzgrxx>li>div>p:last-child{

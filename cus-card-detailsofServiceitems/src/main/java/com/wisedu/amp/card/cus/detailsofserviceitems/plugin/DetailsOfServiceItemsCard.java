package com.wisedu.amp.card.cus.detailsofserviceitems.plugin;

import com.alibaba.fastjson.JSON;
import com.wisedu.amp.card.cus.detailsofserviceitems.model.*;
import com.wisedu.amp.card.cus.detailsofserviceitems.model.appraise.ItemAppraiseDetailSearchForPortalResponse;
import com.wisedu.amp.card.cus.detailsofserviceitems.model.appraise.ItemAppraiseDetailSearchRequest;
import com.wisedu.amp.card.cus.detailsofserviceitems.util.ResultUtil;
...
public class DetailsOfServiceItemsCard extends AbstractCard {
    ...
    public String getCardId () {
        return "CUS_CARD_DETAILSOFSERVICEITEMS";
    }
    ...
    private AppInfoResponse evaluationResponse (CardAjaxRequest request) {
        ...
        //获取热门应用查询接口地址
        logger.debug("开始调用服务事项详情取评价接口...,url=/casp-sim/serviceItem/selectById,参数=" + JSON.toJSONString(entity));
        try {
            response = RestTemplateUtils.post("/casp-sim/serviceItem/selectById", entity, AppInfoResponse.class).getBody();
    ...
    private AmpBaseResponse evaluation (CardAjaxRequest request) {
        ...
        evaluationRequest.setUserId(user.getWid());
        evaluationRequest.setUserName(user.getUserName());
        evaluationRequest.setServiceWid(request.getParam().get("appId"));
        evaluationRequest.setServiceManner(Integer.parseInt(request.getParam().get("serviceManner")));
        evaluationRequest.setServiceSpeed(Integer.parseInt(request.getParam().get("serviceSpeed")));
        evaluationRequest.setTotalSatisfy(Integer.parseInt(request.getParam().get("totalSatisfy")));
        evaluationRequest.setEventIntegrity(Integer.parseInt(request.getParam().get("eventIntegrity")));
        evaluationRequest.setSuggest(request.getParam().get("suggest"));

        HttpEntity entity = new HttpEntity(evaluationRequest);

        AmpBaseResponse response = null;
        logger.debug("调用评价服务事项接口,usr = /itemAppraise/save" + "参数 = " + JSON.toJSONString(entity));
        try {
            response = RestTemplateUtils.post("/casp-sim/itemAppraise/save", entity, EvaluationResponse.class).getBody();
    ...
    private AmpBaseResponse collectApp (CardAjaxRequest request) {
        ...
        try {
            response = RestTemplateUtils.post("/casp-sim/itemManager/favoriteItem", entity, EvaluationResponse.class).getBody();
...
//            }
            ...
            List<String> link;
            if ( null != response.getData().getLinkServices() ) {
                link = response.getData().getLinkServices();
...
//            favoite.setIsRelate("1");
        ...
        try {
            favoriteResponse = RestTemplateUtils.post("/casp-sim/userItemFavoriteList", favoriteRequest, FavoriteResponse.class).getBody();
    ...
    public Object getAppraisement(CardAjaxRequest request){
        ...
        try {
            response = RestTemplateUtils.post("/casp-sim/itemAppraise/queryDetailForPortalList", detailSearchRequest, ItemAppraiseDetailSearchForPortalResponse.class).getBody();
        ...
        if(null!=response && null != response.getData() ){
            ...
            appraiseMap.put("data",response.getData().getItemAppraiseDetailList());
            Map<String,Object> map=new HashMap<>();
            map.put("badAppraiseHidden", response.getData().getBadAppraiseHidden());
     ...
     * @param request
     * @throws
     * @return com.wisedu.amp.card.sys.detailsofserviceitems.model.appraise.ItemAppraiseDetailSearchRequest
     * @date 2021/6/15 7:32
     */

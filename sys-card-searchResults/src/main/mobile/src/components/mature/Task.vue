<template>
  <div>
    <div class="task-list">
      ...
      <div>
        <template v-for="(item, index) in list">
          <div
            class="taskItem portal-font-color-lv2"
          ...
          >
            <div class="sub-subject">
              <div class="taskInfo-topLeft">
                ...
                ></span>
                <div
                  class="taskInfo-type ellipsis portal-primary-color-lv1 portal-primary-backgroundcolor-lv5"
            ...
            <template v-if="item.isDelete !== 1">
              <div class="b-flex sub-info portal-font-color-lv3">
                <span class="ellipsis">{{ $Lan(lanFunName, "filterApps", "所属应用") }}</span
                >：<span class="ellipsis">{{ item.serviceName || item.appName || "-" }}</span>
              </div>
              <div class="b-flex sub-info portal-font-color-lv3">
                <div class="sub-item">
                  <span class="title ellipsis">{{ $Lan(lanFunName, "sponsor", "发起人") }}</span
                  >：<span class="content ellipsis">{{ item.initiatorName || "-" }}</span>
                </div>
              </div>
              <div class="sub-info b-flex portal-font-color-lv3">
                <!-- 待办 -->
    ...
    <!-- pc任务跳转 -->
    <we-popup v-model="isPcUrlShow" round position="bottom" :style="{ height: '80%' }">
      ...
      <pc-link :pcViewUrl="pcViewUrl" :lanFunName="lanFunName" @closePc="isPcUrlShow = false" />
    </we-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    pageAdd(isAdd) {
      if (isAdd) {
        console.log('%c [ this.hasExpand ]-181', 'font-size:13px; background:pink; color:#bf2c9f;', this.hasExpand)
        if (this.hasExpand) {

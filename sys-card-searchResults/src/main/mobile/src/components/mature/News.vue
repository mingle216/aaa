<template>
  <div class="new_list">
    <div class="title portal-font-color-lv3" style="margin-bottom: 12px">{{ name }} ({{ total }})</div>
    <div class="filter_wrap">
      <span
        :class="{
          filter_btn: true,
          'portal-font-color-lv1': sortType !== item.key,
          'portal-primary-backgroundcolor-lv1': sortType === item.key,
        }"
      ...
      <template v-for="(item, index) in list">
        ...
        >
          <div class="image_item" v-if="item.headpicurl">
            <div class="flex_wrap">
              <div class="wrap">
                ...
                <span class="column_time portal-font-color-lv3">
                  <i class="text_ellipsis column_name" v-html="highLight(item.channel)"> </i>
            ...
            <template v-if="currentUser && showCollectNews">
              ...
              />
              <we-icon v-else name="star" color="#FFBC00" @click.stop="handleCollect(item)" />
            </template>
          </div>
          <div class="no_image_item" v-else>
            ...
            <p class="column_time portal-font-color-lv3">
              <span class="text_ellipsis column_name" v-html="highLight(item.channel)"> </span>
              <span>
                {{ item.publishTime && item.publishTime.slice(0, -3) }}
              </span>
            </p>

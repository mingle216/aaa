<template>
  <div style="position: relative">
    <div class="newsTab " v-if="showHeader">
      <div class="newsTab__left portal-font-color-lv2" ref="NewsTab">
        ...
        >
          <div
            ...
            :key="item.wid"
            :class="[
              item.wid === selectedTab ? 'active' : 'portal-primary-color-hover-lv1',
            ]"
      ...
      <div
        class="newsTab__right portal-font-color-lv2"
        v-if="showMore || showSubscribe"
        :class="[showMore ? 'moreShadow' : '']"
      >
        <span
          class="newsTab__right__btn"
          @click.stop="handleClickMore"
          v-if="showMore"
          ><span
            class="text__ellipsis"
            style="max-width: 54px;vertical-align: middle;"
            >{{ $Lan("SYS_CARD_NEWSANNOUNCEMENT", "more", "更多") }}</span
          ><i
            class="iconfont icon-Down newsTab__right__MoreIcon portal-font-color-lv4"
            :class="[expand ? 'expand' : '']"
          ></i
        ></span>
        <span
          class="newsTab__right__split"
          v-if="showMore && showSubscribe"
        ></span>
        <span
          class="newsTab__right__btn portal-primary-color-lv1"
          v-if="showSubscribe"
          @click="handleSubscribe"
          ><i
            v-if="!showMore"
            class="iconfont icon-icon-dingyue newsTab__right__subscribeIcon"
          ></i
          ><span
            class="text__ellipsis"
    ...
    <!-- 更多栏目 -->
    <div
      class="newsTab__more portal-font-color-lv1"
      :class="[expand ? 'show' : '']"
      @click.stop
    >
      <ContainerScroll :maxHeight="cardHeight * 0.8" :barKeepShow="true">
        <div
          class="newsTab__more__item portal-primary-color-hover-lv1 portal-primary-border-color-hover-lv1"
          v-for="item in subscribeListsAll"
          :key="item.wid"
          :class="[
    ...
    <!-- 订阅modal -->
    <we-dialog
      :title="$Lan('SYS_CARD_NEWSANNOUNCEMENT', 'programLists', '栏目列表')"
      :append-to-body="true"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
      :visible.sync="visible"
      custom-class="newsSubscibeModal"
      top="15vh"
      width="800px"
      @closed="handleClickTrack()"
    >
      <subscribe-modal
        :dataLists="configureLists"
      ...
      <div slot="footer" class="dialog-footer">
        <we-button
          class="portal-primary-color-hover-lv1 portal-primary-border-color-hover-lv1 portal-primary-backgroundcolor-hover-lv5"
          @click="handleClose"
          >{{ $Lan("SYS_CARD_NEWSANNOUNCEMENT", "cancel", "取 消") }}</we-button
        >
        <we-button
          type="primary"
          :class="[
          ...
          :disabled="disabled"
          @click="handleOk"
          >{{ $Lan("SYS_CARD_NEWSANNOUNCEMENT", "ok", "确 定") }}</we-button
        >
      </div>
    </we-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async getConfiguredChannels(isInit, isUpdate) {
      this.loading = true;
      isInit && this.$parent.changeLoading();
      await window.shell.execCardMethod(
        {
          cardId: this.router.cardId,
          cardWid: this.router.cardWid,
          method: "getConfiguredAndSubscribedChannel",
        },
        (res) => {
          this.loading = false;
          isInit && this.$parent.changeLoading();
          this.configureLists = res.data?.configuredChannel || [];
          this.subscribeLists = res.data?.subscribedChannel || [];
          if (isUpdate) {
            this.updateTabLists();
          } else {
          ...
          isInit && this.$parent.loadedEndTrack();
        }
      );
    },
    ...
    handleOk() {
      const checkedLists = this.$refs.SubscribeModal.checkedLists || [];
      const channelIds = checkedLists
        .filter((el) => el.isChannel)
        .map((item) => item.id);
      const programIds = checkedLists
        .filter((el) => !el.isChannel)
        .map((item) => item.id);
      this.okLoading = true;
      window.shell.execCardMethod(
        {
          ...
          param: {
            ...
            programIds: programIds && programIds.join(","),
          },
        },
        (res) => {
    ...
    handleUpdateEvents() {
      this.getConfiguredChannels(false, true);
    },
  },
  async mounted() {
    // 获取后台配置的栏目
    await this.getConfiguredChannels(true);

<template>
  <div class="newsAnnouncement" ref="NewsAnnouncement" v-loading="loading">
    ...
    <TabNav
      ...
      :newsShowSubscribe="config.newsShowSubscribe"
      :isScribeProgramme="isScribeProgramme"
      @change-tab="handleChangeTab"
    ></TabNav>
    <template v-if="newList && newList.length > 0">
      <div v-loading="newsLoading">
        <!-- 图片、标题、时间及摘要 -->
        <div class="cus-img-wrap" v-if="config.newsDisplay && config.newsDisplay.type == 2">
          <div class="cus-img-title-left" style="margin-right: 16px;">
            <div class="cus-img-first" v-for="(item,index) in newList.slice(0,1)" :key="index">
              <div class="first-img-box">
                <img :src="item.picUrl" alt="">
                <div class="cus-tag-box">
                  <div class="tag newTag" v-if="item.newDisplay" alt="" />
                  <div class="tag topTag" v-if="item.isTop" alt="" />
                </div>
              </div>
              <div class="cus-img-detail">
                <div class="news-name" @click="goLink(item)">
                  <v-clamp
                ...
                <div class="new-sub">
                  <v-clamp
                          ...
                          autoresize
                          :max-lines="2"
                  >{{ item.abstractVal || item.contents || "-" }}</v-clamp
                  >
                </div>
                <div class="news-times">
                  <span>来源:{{ `${item.columnName}${item.channelName}` || "-" }}</span>
                  <span>{{ item.pubTime || "-" }}</span>
                </div>
              </div>
            </div>
            <div class="cus-img-second" v-for="(item,index) in newList.slice(1,2)" :key="index">
              <div class="cus-seccond-top" @click="goLink(item)">
                <div class="ellipsis second-title">{{ item.title && item.title.replace(/\n/g, " ") }}</div>
                <div class="tag newTag" v-if="item.newDisplay" alt="" />
                <div class="tag topTag" v-if="item.isTop" alt="" />
              </div>
          ...
          <div class="cus-img-title-left">
            <div class="cus-img-second" v-for="(item,index) in newList.slice(2,5)" :key="index">
              <div class="cus-seccond-top" @click="goLink(item)">
                <div class="ellipsis second-title">{{ item.title && item.title.replace(/\n/g, " ") }}</div>
                <div class="tag newTag" v-if="item.newDisplay" alt="" />
                <div class="tag topTag" v-if="item.isTop" alt="" />
    ...
    <EmptyCon
          ...
          showSubscribe && config.newsShowSubscribe == 1
            ? ''
            : $Lan('CUS_CARD_CQNEWS', 'noData', '暂无内容')
        "
            :subTip="
          showSubscribe && config.newsShowSubscribe == 1
            ? $Lan('CUS_CARD_CQNEWS', 'noData', '暂无内容')
    ...
    >
      <template v-slot:operate>
        <div
                v-if="showSubscribe && config.newsShowSubscribe == 1"
                class="portal-primary-color-lv1 subscribeBtn"
        >
            <span @click="handleSubscribe">{{
              $Lan("CUS_CARD_CQNEWS", "goSubscribe", "点击订阅")
            }}</span>
        </div>
      </template>
    </EmptyCon>
    ...
    >
            <span class="ellipsis">{{
              $Lan("CUS_CARD_CQNEWS", "showMore", "查看更多")
            }}</span>
      <i
              class="iconfont icon-menu-Unfold newsAnnouncement__page-icon"
      ></i>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      isScribeProgramme: false,
      subscribeLists: []  // 已订阅频道/站点
    };
  },
  computed: {
    showMore() {
  ...
  methods: {
    ...
    handleChangeTab(channelIds, subscribeLists) {
      console.log('%c [ subscribeLists ]-247', 'font-size:13px; background:pink; color:#bf2c9f;', subscribeLists)
      console.log('%c [ handleChangeTab channelIds ]-245', "color:#bf2c9f;", channelIds)
    ...
    renderData() {
      ...
      const wids = this.channelIds.split(',')
      const channelIds = this.subscribeLists.filter(el => el.type == 0).map(el => el.wid)   // 频道
      const programIds = this.subscribeLists.filter(el => el.type == 1).map(el => el.wid)   // 站点
      window.shell.execCardMethod(
        {
          ...
          param: {
            channelIds: wids.filter(el => channelIds.includes(el)).join(','),
            programIds: wids.filter(el => programIds.includes(el)).join(','),
            pageNumber: 1,
          },
        },
        (res) => {
          ...
          if (res.errcode === "0") {
            ...
            this.isScribeProgramme = this.config.subscribeProgramme == '1'
            // 是否开启后端记录已读未读
            if (!this.config.isEnableRead) {

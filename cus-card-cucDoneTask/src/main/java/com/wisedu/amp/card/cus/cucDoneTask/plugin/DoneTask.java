package com.wisedu.amp.card.cus.cucDoneTask.plugin;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.wisedu.amp.card.cus.cucDoneTask.model.*;
 ...
 */
@MinosSPI
public class DoneTask extends AbstractCard {
    private final static Logger logger = LogManager.getLogger(DoneTask.class);
     ...
     * @author jszhang
     */
    private static final String TASK_CENTER_MOBILE_DONE_TASK_URL_SUFFIX ="/taskCenter/*default/mobile.do#/DoneTask";

    @Override
    public String getCardId() {
        return "CUS_CARD_CUCDONETASK";
    }
...
//        user.setUserAccount("ampadmin");
        ...
        try {
            //查询任务来源和数量列表
            if (null != config.getSourceList() && ResultUtil.RadioFlag.YES.getIndex().equals(config.getSourceList().toString())) {
            ...
            logger.error("已办任务列表出错：接口查询异常！", e);
            t.put(ResultUtil.SOURCELIST, new ArrayList<TaskInfo>());
        }
        ...
        if (isMobileDevice) {
            String url = ApplicationContextUtil.get(SystemConfigService.class).getSystemConfigValue("taskcenter_url")
                    + TASK_CENTER_MOBILE_DONE_TASK_URL_SUFFIX;
    ...
    private List<TaskInfo> getSourceList(String taskCenterUrl, UserInfo user) {
        ...
        taskInfoRequest.setFlag(ResultUtil.TaskType.DONE.getIndex());
    ...
    private List<TaskInfo> getTaskList(String taskCenterUrl, UserInfo user, ConfigInfo config, String sourceWid) {
        ...
        List<TaskInfo> taskList = new ArrayList<>();
        if (org.apache.commons.lang3.StringUtils.isEmpty(taskCenterUrl) || org.apache.commons.lang3.StringUtils.isEmpty(path)) {
            return taskList;
        }
        try {
    ...
    private String config(CardAjaxRequest request) {
        ...
        t.binding(ResultUtil.CARDID, getCardId());
        t.binding(ResultUtil.CARDWID, request.getCardWid());
        String str = t.render();
        return str;
    }
    ...
    private Integer getMarkNumber() {
        ...
        UserInfo user = ApplicationContextUtil.get(UserUtil.class).getCurrentUser();
        if (null == user) {
            logger.error("已办任务数量查询出错：用户获取失败！");
            return ResultUtil.ZERO;
        }
        TaskInfoRequest taskInfoRequest = new TaskInfoRequest();
        taskInfoRequest.setUserId(user.getUserAccount());
        List<String> runPlatformList = new ArrayList<>();
        ...
        try {
            ...
            if (ResultUtil.ResponseStatus.SUCCESS.getIndex().equals(response.getErrcode())) {
                count = response.getData().getDoneCount();
            }
        } catch (Exception e) {
            logger.error("已办任务查询已办数量出错：", e);
        }
        return count;
    }
    ...
    private List<TaskInfo> handleTaskListData(TaskJsonInfoResponse response) {
        List<TaskInfo> taskList = new ArrayList<>();
        if (ResultUtil.ResponseStatus.SUCCESS.getIndex().equals(response.getErrcode())) {
            Map<String, Object> queryTodoList = response.getData().get(ResultUtil.QUERY_DONE_TASK);

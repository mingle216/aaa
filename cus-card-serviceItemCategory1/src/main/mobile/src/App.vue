...
export default {
  ...
  data() {
    ...
    return {
      ...
      lanFunName: "SYS_CARD_SERVICEITEMCATEGORY_h5",
      loading: false,
      serviceItemInfo: [],
      categoryItemInfo: [],
      tabInfo: [],
    ...
    getCardData(isInit) {
      ...
      .then(data=> {
        ...
        if (data.errcode === "0") {
          ...
          // 展示二级分类
          const showLevel = Array.isArray(this.config.resultWay) ? this.config.resultWay[2] || 0 : 0
          if (showLevel == 1 && (!this.params.categoryWid || this.params.categoryWid === 'dept-subject')) {
            let arr = []
            this.serviceItemInfo.forEach(el => {
              if (el.children && el.children.length) {
                const temp = el.children.filter(d => d.itemList && d.itemList.length)
                arr.push(...temp)
              }
            })
            this.serviceItemInfo = arr
          }
          if (
            this.isFirstLoad &&
            this.tabInfo.length &&

...
const defaultCfg = {
  columns: 4, //列数
  content: 0, //内容显示方式0：事项，1:二级分类 2:事项统计
  rows: 2, //行数
  ...
  methods: {
        ...
        (data) => {
          ...
          if (data.errcode === "0") {
            ...
            this.config = {
              ...
              showLevel: Array.isArray(m_data.config.resultWay) ? m_data.config.resultWay[2] || 0 : 0,  // 0:一级部门 1:二级部门
            ...
            this.params.roleWid = data.data.serviceItemInfo[0].roleName
            ...
            if (this.config.showType === 1) {
              ...
              if (this.config.showLevel == 1 && (!this.params.categoryWid || this.params.categoryWid === 'dept-subject')) {
                this.serviceList = this.serviceList.map(el => {
                  let arr = []
                  el.itemModelList.forEach(d => {
                    if (d.children && d.children.length) {
                      const temp = d.children.filter(el => el.itemList && el.itemList.length)
                      arr.push(...temp)
                    }
                  })
                  return {
              ...
              if (this.config.showLevel == 1 && (!this.params.categoryWid || this.params.categoryWid === 'dept-subject')) {
                ...
                this.serviceList.forEach(el => {
                  if (el.children && el.children.length) {
                    const temp = el.children.filter(el => el.itemList && el.itemList.length)
                    arr.push(...temp)
                  }
                })
    ...
    clickItemHandle(item) {
      ...
      window.shell.openPage(
        `/itemCategoryDetail?roleWid=${this.params.roleWid}&oneCategoryWid=${item.categoryWid}&categoryWid=${item.subjectWid}&cardWid=${this.router.cardWid}`,
        1,
        1
      );
    },

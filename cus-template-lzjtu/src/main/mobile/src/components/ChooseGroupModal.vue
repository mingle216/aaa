<template>
  ...
  >
    <div class="modal__title">
      <span class="ellipsis">{{ $Lan('CUS_TEMPLATE_LZJTU_h5', 'goToService', '跳转服务') }}</span>
    ...
    <div class="modal__wrap">
      <div class="group__wrap">
        ...
        <div class="group__title ellipsis" v-if="!hidden">
          {{ $Lan('CUS_TEMPLATE_LZJTU_h5', 'groupTips', '您有如下身份可访问该服务，请选择:') }}
        </div>
        <div class="diable__title" v-else>
          {{ $Lan('CUS_TEMPLATE_LZJTU_h5', 'unavailableService', '此服务暂时无法使用') }}
        </div>
        ...
        <p v-if="hidden">
          {{ $Lan('CUS_TEMPLATE_LZJTU_h5', 'contactAdmin', '如有问题请联系管理员') }}
        </p>
      </div>
    </div>
  </we-popup>

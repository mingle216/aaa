<template>
  ...
  <div class="card-container portal-font-color-lv1" style="min-width: 250px">
    ...
    <AutoContainer
      ...
      :con-height="containerHeight"
      class="auto-tabs"
      v-if="sourceList && sourceList.length && allClassifyData.length > 1"
    >
    <div style="background: #BD1A2D;border-radius: 4px 4px 0px 0px;text-align: center;color: #ffffff;height: 50px;font-size: 16px;line-height: 50px;">主题分类</div>
      <div class="source-list">
        <!-- portal-font-color-lv2 -->
        <div
          class="source-item"
          :class="showChecked(item.typeId)"
          v-for="(item, index) in sourceList"
          :key="index"
          @click="clickService(item.typeId)"
          @mouseenter="mouseenterType(item.typeId)"
          @mouseleave="mouseleaveType(item.typeId)"
        >
          <div
          ...
          >
            ...
            <img v-show="selectedTypeId != item.typeId" style="width: 14px;height: 14px;" src="../assets/wxz.png">
            <img v-show="selectedTypeId === item.typeId" style="width: 14px;height: 14px;" src="../assets/xz.png">
        ...
        <span
          v-if="allClassifyData.length > 1 && isLogin"
          class="setting portal-primary-color-lv1"
          @click="handleSubscribe"
          ><i class="iconfont icon-icon-dingyue"></i
          >{{ $Lan(LanFunName, "operate", "订阅") }}</span
        >
      </div>
    </AutoContainer>

    <EmptyCon
      :tip="$Lan(LanFunName, 'emptytip_data', 'emptytip_data')"
      :height="containerHeight"
      style="width: 100%"
      v-if="sourceList && sourceList.length === 1 && serviceList.length === 0"
    />
    ...
    <AutoContainer
      ...
      "
    >
      <div><input class="searchinput" placeholder="请输入系统名称" v-model="searchVal" /><span class="searchbtn portal-primary-backgroundcolor-lv1" @click="dzsearch()">搜索</span></div>
      <div class="task-list" ref="taskList">
        <!--空数据 -->
        <EmptyCon
          :tip="$Lan(LanFunName, 'emptytip', '该分类下无服务')"
          :height="containerHeight"
          style="width: 100%"
        ...
        <template v-else>
          <AutoContainer>
            <div
              class="rowStyle"
              v-for="(services, pIndex) in servicesData"
              :key="pIndex"
            >
              <div
              ...
              >
                <div
                  class="app-item portal-primary-color-hover-lv1 portal-primary-backgroundcolor-hover-lv5"
                  v-if="!item.serviceTabItemNoShow"
                  :class="[
                    'task-list-item-empty-type',
                    { 'task-list-item': sourceList && sourceList.length > 1 },
                  ]"
                  :serviceName="item.appName"
                  :style="itemWidth"
                  @mouseenter="mouseenter(item.appId)"
                  @mouseleave="mouseleave(item.appId)"
                ...
                >
                  <div class="app-item-card">
                    ...
                    <div
                      class="app-item-appName-Con"
                      :class="getAppNameHoverWidth()"
                    >
                    ...
                    <div class="appfavorite-icon">
                      <div :class="getAppItemColl(item.appId)">
                        <a
                          @click.stop="collectApp(item.appId, item.appFavorite)"
                        >
                          <i
                            v-if="item.appFavorite"
...
export default {
  ...
  methods: {
    dzsearch(){
      let allID=this.sourceList.filter(item =>{
        if(item.typeName.indexOf('全部')!=-1) return item
      })
      this.clickService1(allID[0].typeId)
    },
    mouseenter(appId) {
    ...
    async clickService1(typeId) {
      this.selectedTypeId = typeId;
      let obg={
        typeId:typeId,
        searchVal:this.searchVal
      }
      this.$emit("search-val", obg);
      this.handleClickTrack({
        infoType: 8,
        filterInfo: {
          typeId

<template>
  <div
    ...
    ref="servicebus"
  >
  <div style="text-align: center;margin: 25px 0;" class="dzztc">
    <img src="./assets/xtztc.png">
  </div>
    <ServiceTab
      v-if="tabPosition === '1'"
      :sourceList="classifyData"
      :serviceList="appData"
      :inSelectedTypeId="selectedTypeId"
      @click-type="clickType"
      @collect-app="collectApp"
      @search-val="searchVal"
      :containerType="containerType"
      :containerHeight="containerHeight"
      :appLoading="appLoading"
      :servicesData="servicesData"
      :allClassifyData="allClassifyData"
    ...
    ></serviceTransverse>
    <service-topics
      v-model="isShowTopics"
      :sourceList="allClassifyData"
      :card="card"
      :router="router"
    ></service-topics>
  </div>
...
export default {
  ...
  methods: {
    ...
    renderData1(obg, init) {
      console.log(obg.searchVal)
      let param = obg.typeId ? { typeId:obg.typeId } : {};
      window.shell.execCardMethod(
        ...
        (res) => {
          ...
          if (res.errcode === "0") {
            ...
            this.appData = res.data.appData.filter(item =>{
                if(item.appName.indexOf(obg.searchVal)!=-1) return item 
              });
            ...
            this.selectedTypeId = obg.typeId
              ? obg.typeId
  ...
  computed: {
    getItemWidth() {
      ...
      let f_w =
        this.tabPosition === "0"
          ? this.containerWidth
          : Number(this.containerWidth) - 252;

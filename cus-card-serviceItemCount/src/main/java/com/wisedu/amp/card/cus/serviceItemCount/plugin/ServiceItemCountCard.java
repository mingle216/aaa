package com.wisedu.amp.card.cus.serviceItemCount.plugin;


import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.google.common.collect.Lists;
import com.wisedu.amp.card.cus.serviceItemCount.model.*;
import com.wisedu.amp.card.cus.serviceItemCount.util.ConstantUtil.SERVICE_ITEM;
import com.wisedu.amp.card.cus.serviceItemCount.util.PageUtils;
import com.wisedu.amp.card.cus.serviceItemCount.util.ServiceItemCountUtil;
...
public class ServiceItemCountCard extends AbstractCard {
    ...
    public String getCardId() {
        return "CUS_CARD_SERVICEITEMCOUNT";
    }
    ...
    public ComponentContainer getConfig(CardConfigReq cardConfigReq) {
        ...
        String url = "/coosk/itemManager/roleList";
    ...
    private CardResponse getStatistics(CardAjaxRequest request) {
       ...
       /* Map<String, String> headers = new HashMap<>();
        headers.put("Content-Type", "application/json");*/
        String url = "/coosk/itemManager/queryItemStatistics2";
        Map<String, String> res =new HashMap<>();
        Map<String, List<MultiLangData>> roleInfo =new HashMap<>();
        try {
        ...
        if (allServiceCount != 0) {
            ...
            res.put(SERVICE_ITEM.ONLINE_PERCENT.getWid(), count + "%");
        } catch ( Exception e ) {
            logger.error("事项统计处理发生异常:",e);
        }
        //--------------------------------------事项统计处理结束---------------------------------------
        try {
            ...
            res.put(SERVICE_ITEM.ALL_COUNT.getWid(), allCount + "");
            res.put(SERVICE_ITEM.CURRENT_COUNT.getWid(), handleFlowCountList.getRunningCount() + "");
            res.put(SERVICE_ITEM.DONE_COUNT.getWid(), handleFlowCountList.getCompleteCount() + "");
        }catch ( Exception e ){
            logger.error("任务中心处理发生异常:"+e);
        }
        //--------------------------------------任务中心处理结束---------------------------------------
        List<ItemStatisticsInfo> result = Lists.newArrayList();
        try {
            for (ServiceItem i : cardConfig.getDataItemList()) {
                if (org.apache.commons.lang3.StringUtils.isNotEmpty(res.get(i.getDataId()))) {
                    ...
                    info.setCount(res.get(i.getDataId()));
                    result.add(info);
                }
            }
        } catch ( Exception e ) {
            logger.error("result处理发生异常:"+e);
        }
        CardResponse resp = new CardResponse();
        resp.setServiceItemList(result);
        resp.setColumns(cardConfig.getSelect());
        return resp;
    }

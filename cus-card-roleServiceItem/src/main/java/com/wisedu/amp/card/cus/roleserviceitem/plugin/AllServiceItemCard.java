package com.wisedu.amp.card.cus.roleserviceitem.plugin;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import com.wisedu.amp.card.cus.roleserviceitem.enums.DisplayTypeEnum;
import com.wisedu.amp.card.cus.roleserviceitem.enums.ItemDisplayInfoEnum;
import com.wisedu.amp.card.cus.roleserviceitem.model.*;
import com.wisedu.amp.card.cus.roleserviceitem.util.AllServiceItemUtil;
import com.wisedu.amp.card.cus.roleserviceitem.util.ChinesePinyinUtil;
...
public class AllServiceItemCard extends AbstractCard {
    ...
    public String getCardId() {
        return "CUS_CARD_ROLESERVICEITEM";
    }

    @Override
    public void destroy() {
        logger.debug("card[CUS_CARD_ROLESERVICEITEM] destroyed ...");
    }
     ...
     * @param
     * @throws
     * @return java.util.List<com.wisedu.amp.card.cus.roleserviceitem.enums.ItemDisplayInfoEnum>
        ...
        if(DisplayTypeEnum.TILE.getCode().equals(displayType)){
            if (CollectionUtils.isNotEmpty(list.getData())) {
                ...
                if(CollectionUtils.isNotEmpty(serviceItemList)){
                    int itemGroupList= serviceItemList.stream().filter(k -> CollectionUtils.isNotEmpty(k.getDatas())).mapToInt(k -> k.getDatas().size()).sum();
                    result.put("total",itemGroupList);
                }
            }
        }else{
    ...
    private AllServiceItemRequest getAllserviceItemRequest(Config config, CardAjaxRequest request) {
        ...
        allserviceItemRequest.setUserAccount(getCurrentUserAccount());
    ...
    private void getModelsByGroup(AllServiceItemRequest allserviceItemRequest, Config config, List<AllServiceItemResponse.ServiceItemModel> allServiceItems,
        ...
        if (CollectionUtils.isNotEmpty(otherServiceItem)) {
            ...
            contentModel.setNavName("#");
            contentModel.setNavValue("#");

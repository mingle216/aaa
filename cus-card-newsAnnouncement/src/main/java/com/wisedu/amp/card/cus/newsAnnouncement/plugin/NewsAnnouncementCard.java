package com.wisedu.amp.card.cus.newsAnnouncement.plugin;
...
import com.wisedu.amp.card.cus.newsAnnouncement.common.IaInfo;
import com.wisedu.amp.card.cus.newsAnnouncement.model.NewDisplayEnum;
import com.wisedu.amp.card.cus.newsAnnouncement.model.NewsColumnAndChannelEnum;
import com.wisedu.amp.card.cus.newsAnnouncement.model.NewsColumnsAndChannelVo;
import com.wisedu.amp.card.cus.newsAnnouncement.model.NewsVo;
import com.wisedu.amp.card.cus.newsAnnouncement.model.SubscribedChannelVo;
import com.wisedu.amp.card.cus.newsAnnouncement.model.TimeDisplayEnum;
...
public class NewsAnnouncementCard extends AbstractCard {
    ...
    public String getCardId() {
        return "CUS_CARD_NEWSANNOUNCEMENT";
    }
    ...
    private Object getChannelNews(CardAjaxRequest request) {
        ...
        logger.info("========1处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        //请求新闻接口，封装数据
        long pageNumber;
        long pageSize;
        ...
        logger.info("========2处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        if (pageRes == null) {
            ...
            logger.info("========3处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
...
//            throw new BusinessException("用户未登录，不可以执行订阅操作");
    ...
    private Object getSubscribedChannels(CardAjaxRequest request) {
        ...
        List<String> localIds = newsContent.stream()
            .map(e -> ((JSONObject) e).getString("id"))
            .collect(Collectors.toList());
        logger.info("========getSubscribedChannels1处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        //2.调用接口回去订阅频道
        UserInfo currentUser = ApplicationContextUtil.get(UserUtil.class).getCurrentUser();
        DubboCardChannelRequest dubboCardChannelRequest = new DubboCardChannelRequest();
        dubboCardChannelRequest.setCardWid(cardWid);
        ...
        logger.info("========getSubscribedChannels2处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        if (cardChannel == null) {
            ...
            logger.info("========getSubscribedChannels3处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));

            List<NewsColumnsAndChannelVo> tree = new NewsAnnouncementCard().buildTree(vos);
            logger.info("========getSubscribedChannels4处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
            List<NewsColumnsAndChannelVo> sortList = new NewsAnnouncementCard().treeToList(tree);
            logger.info("========getSubscribedChannels5处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
            List<String> subscribedWids = cardChannel.getData().stream().filter(e->localIds.contains(e.getChannelWid())).map(DubboSubscribeVo::getChannelWid)
                .collect(Collectors.toList());
            logger.info("========getSubscribedChannels6处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
            return sortList.stream().filter(e -> subscribedWids.contains(e.getId()))
                .map(e -> new SubscribedChannelVo()
                    .setName(e.getName())
                    .setFixed(fixedIds.contains(e.getId()))
                    .setWid(e.getId())).collect(Collectors.toList());
            } else {
    ...
    public ComponentContainer getConfig(CardConfigReq cardConfigReq) {
      ...
      if(value instanceof List && !((List) value).isEmpty()){
        ...
        List<HashMap> collect = sortList.stream().filter(e -> localId.contains(e.getId())).map(e -> {
            HashMap hashMap = new HashMap();
            hashMap.put("id", e.getId());
            hashMap.put("fixed", localFixedId.contains(e.getId()));
            return hashMap;
        }).collect(Collectors.toList());
        ...
        if (IaInfo.IA_SUCCESS_CODE.equals(newsDubboRes.getErrcode())) {
            return newsDubboRes.getData().stream().map(e ->
                new NewsColumnsAndChannelVo().setId(e.getWid())
                    ...
                    .setName(getName(e.getName(),lang))
                    .setSort(e.getSort())
            ).collect(Collectors.toList());
        }
    ...
    public String getName(List<DubboMultiLangVo> langList,String currentLang) {
        ...
        UserInfo currentUser = ApplicationContextUtil.get(UserUtil.class).getCurrentUser();
        if(null!=currentUser){
            currentLang=currentUser.getPreferredLanguage();
        }else{
            currentLang=StringUtil.isNotEmpty(currentLang)?currentLang:Global.DEFAULT_LANGUAGE;
        }
     ...
     * @throws
     * @return java.util.Set<com.wisedu.amp.card.sys.newsAnnouncement.model.NewsColumnsAndChannelVo>
    ...
    public List<NewsColumnsAndChannelVo> filterUserColumnsAndChannel(List<NewsColumnsAndChannelVo> remoteNews,
        ...
        List<NewsColumnsAndChannelVo> localColumnNames = remoteNews.stream().filter(e -> allColumns.contains(e.getId()))
            .collect(Collectors.toList());
        logger.info("========filterUserColumnsAndChannel1处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        localColumnNames.forEach(e -> findParent(e, remoteNews, parents));
        logger.info("========filterUserColumnsAndChannel2处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        List<String> existParentIds = parents.stream().map(NewsColumnsAndChannelVo::getId)
            .collect(Collectors.toList());
        logger.info("========filterUserColumnsAndChannel3处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        ...
        logger.info("========filterUserColumnsAndChannel4处耗时>>>"+String.valueOf(System.currentTimeMillis()-start));
        return collect;
    }

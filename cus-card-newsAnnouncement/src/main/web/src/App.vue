<template>
  <div class="newsAnnouncement" ref="NewsAnnouncement" v-loading="loading">
    ...
    <TabNav ref="TabNav" :router="router" :cardWidth="cardWidth" :cardHeight="containerHeight"
      :newsShowSubscribe="config.newsShowSubscribe" @change-tab="handleChangeTab">
    </TabNav>
    <AutoContainer :scroll="scroll" :con-type="containerType" :con-height="containerHeight">
      <template v-if="newList && newList.length > 0">
        <div v-loading="newsLoading">
          ...
          <div class="newsAnnouncement__page portal-primary-color-lv1" v-if="showMore" @click="handleShowMore">
            <span class="ellipsis">{{
              $Lan("SYS_CARD_NEWSANNOUNCEMENT", "showMore", "查看更多")
            }}</span>
      ...
      <EmptyCon v-else :tip="
        showSubscribe && config.newsShowSubscribe == 1
          ? ''
          : $Lan('SYS_CARD_NEWSANNOUNCEMENT', 'noData', '暂无内容')
      " :subTip="
  showSubscribe && config.newsShowSubscribe == 1
    ? $Lan('SYS_CARD_NEWSANNOUNCEMENT', 'noData', '暂无内容')
    : ''
        ...
        <template v-slot:operate>
          <div v-if="showSubscribe && config.newsShowSubscribe == 1" class="portal-primary-color-lv1 subscribeBtn">
            <span @click="handleSubscribe">{{
              $Lan("SYS_CARD_NEWSANNOUNCEMENT", "goSubscribe", "点击订阅")
            }}</span>
          </div>
        </template>
      </EmptyCon>
...
export default {
  ...
  data() {
    return {
      ...
      containerHeight: 412, // 高度 只有当heightFlag为1,2时，此值才生效
      cardWidth: 0,
      loading: false,
      newsLoading: false,
      current: 1,
  ...
  computed: {
    ...
    emptyHeight() {
      ...
      return this.containerType !== 0 && height < 366 ? height : 366;
    },
    switchSize() {
  ...
  methods: {
      ...
      window.shell.execCardMethod({
        ...
        if (data.errcode === "0") {
          ...
          if (typeof datas === 'string') {
            ...
            this.showPage = config.showPage === 1 ? true : false
          }
        }
      })
    },
      ...
      window.shell.execCardMethod(
        ...
        (res) => {
          if (res.errcode === "0") {
            ...
            if (this.config.cardType == "NoOA") {
              this.renderData();
            } else {
              this.config.newsDisplay.type == 4
              ...
              this.getDataType(this.config.cardType, this.config.newsTotal)
    ...
    getDataType(type, pageSize) {
      this.newsLoading = true;
      window.shell.execCardMethod(
        {
          cardId: "CUS_CARD_MYNEWS",
          cardWid: "41603402949833",
          method: type,
          param: { "lang": "zh_CN", "pageSize": pageSize.toString(), "pageNum": "1" }
        },
        (res) => {
          ...
          if (res.errcode === "0") {
            this.newList = []
            if (type == 'getListMessages') {
              res.data.datas.forEach(val => {
                this.newList.push({
                  title: val.subject,
                  isTop: false,
                  newDisplay: false,
                  pubTime: this.toData(val.receivedDate),
                  url: val.detailUrl ? val.detailUrl : ''
                })
              })

            } else if (type == 'getEmailReceiveList') {
              res.data.datas.message.list.forEach(val => {
                this.newList.push({
                  title: val.TITLE,
                  isTop: false,
                  newDisplay: false,
                  pubTime: val.SENDOR_UNIT_NAME,
                  url: val.RESOURCE_ID ? 'http://i.cuc.edu.cn/cucdcp/forward.action?path=/portal/portal&p=oaemail&ac=1#m=oaemail&pt=de&ptc=d&code=' + val.RESOURCE_ID + '&pn=1&status=1' : ''
                })
              })
            } else if (type == 'getUserPimList') {
              res.data.datas.message.list.forEach(val => {
                this.newList.push({
                  title: val.PIM_TITLE,
                  isTop: false,
                  newDisplay: false,
                  pubTime: val.CREATE_TIME.split(' ')[0].split('-')[0] + '/' + val.CREATE_TIME.split(' ')[0].split('-')[1],
                  day: val.CREATE_TIME.split(' ')[0].split('-')[2],
                  url: val.RESOURCE_ID ? 'http://i.cuc.edu.cn/cucdcp/forward.action?path=/portal/portal&p=oaemail&ac=1#m=oaemail&pt=de&ptc=d&code=' + val.RESOURCE_ID + '&pn=1&status=1' : ''
                })
              })
            } else {

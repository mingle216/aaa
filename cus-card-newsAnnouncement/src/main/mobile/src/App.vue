    ...
    >
          ...
          >
            <div class="newsAnnouncement-listFather">
              ...
              >
                ...
                >
                  <span class="ellipsis">{{
                    $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "showMore", "查看更多")
                  }}</span>
                  <i
      ...
      <EmptyCon v-else :height="179">
        <we-button
          ...
          {{
            $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "goSubscribe", "点击订阅")
          }}</we-button
        >
        <p class="portal-font-color-lv2">
          {{ $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "noChannel", "暂无栏目") }}
        </p>
      </EmptyCon>
    </div>
    <subscribe
      ...
      :router="router"
      @renderTab="getTabs"
      @updateTab="(val) => (configChannelNum = val)"
    ></subscribe>
  </div>
</template>
  ...
  methods: {
    ...
    filterTime(time) {
      ...
      let TimeArr = [
        {
          time: 15 * 60,
          text: this.$Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "timeJust", "刚刚"),
        },
        {
          time: 60 * 60,
          text: (time) =>
    ...
    getTabs() {
      //获取订阅栏目
      this.execCardMethod(
        { ...this.card, method: "getSubscribedChannels" },
        (data) => {
          this.tabs = data.data || [];
          this.reset();
    ...
    setAll() {
      if (this.tabs.length > 1) {
        this.tabs.unshift({
          name: this.$Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "allChannle", "全部"),
          wid: this.tabs
            .map((v) => {
              return v.wid;
            })
        ...
        (res) => {
          ...
          if (!this.config.isEnableRead) {
            list = list.map((v) => {
              return {
                ...v,
                read: this.newsRead.find((curr) => curr === v.url) ? true : false,
              };
            });
          }

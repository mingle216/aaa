      ...
      <header>
        <div class="header-top">
          <h3 class="portal-font-color-lv1">
            {{
              $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "mySubscribe", "我的订阅")
            }}
          </h3>
      ...
      <main>
        <div class="main-top">
          ...
          <div class="top-right" v-if="list.length">
            <we-button
              ...
              @click="handleClose"
              >{{
                $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "cancel", "取消")
              }}</we-button
            >
            <we-button
              ...
              >{{
                $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "ok", "完成")
              }}</we-button
            >
          </div>
        </div>
        <template v-if="list.length">
          <div class="addChannel portal-primary-color-lv1" @click="add">
            ...
            ><span class="text__ellipsis">{{
              $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "addChannel", "新增栏目")
            }}</span>
          </div>
        ...
        <template v-else>
          <EmptyCon
            :tip="$Lan('SYS_CARD_NEWSANNOUNCEMENT_h5', 'noChannel', '暂无栏目')"
          ></EmptyCon>
          <we-button
          ...
          <div class="updateChannel-content">
                      ...
                      <div class="list-left">
                        <div class="list-check" @click="clickChecked(item)">
                          <div
                            class="check-son"
                            :class="
                              className[item.isFixed ? 1 : 0][item.checked]
                            "
                          ></div>
                        </div>
            ...
            <footer>
              <we-button
                ...
                @click="submit"
                >{{
                  $Lan("SYS_CARD_NEWSANNOUNCEMENT_h5", "submit", "我选好了")
                }}</we-button
              >
            </footer>
          </div>
  ...
  data() {
    ...
    return {
      ...
      className: [
        ...
        ["", "fixChecked fixBg", "fixSelective fixBg"],
      ],
    };
  },
  created() {
    ...
    getAllList() {
      this.execCardMethod(
        ...
        (resp) => {
          ...
          const channel = data.filter((item) => item.isChannel);
          this.$emit("updateTab", channel.length);
      ...
      this.execCardMethod(
        {
          ...
          param: { channelIds: this.list.map((v) => v.id).join(",") },
        },
        (data) => {
          if (data.errcode === "0") {

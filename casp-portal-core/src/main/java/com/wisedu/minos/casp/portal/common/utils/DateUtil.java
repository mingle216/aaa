...
public class DateUtil {
    ...
    public static final String DATE_FORMATE_STRING_L = "HH:mm:ss";
    public static final String DATE_FORMATE_STRING_M = "yyyy-MM-dd HH:mm";
    // 日期格式yyyy-MM-dd
    ...
    private static SimpleDateFormat dateTimeInstance() {
        ...
        if (df == null) {
            df = new SimpleDateFormat(DATE_FORMATE_STRING_A);
    ...
    private static SimpleDateFormat dateInstance() {
        ...
        if (df == null) {
            df = new SimpleDateFormat(DATE_FORMAT_YYYY_MM_DD);
            ThreadDate.set(df);
        }
        return df;
    }
    ...
    private static SimpleDateFormat timeInstance() {
        ...
        if (df == null) {
            df = new SimpleDateFormat(DATE_FORMATE_STRING_L);
            ThreadTime.set(df);
        }
        return df;
    }
    ...
    /**
     * 获取下周的结束时间，周日的23:59:59
     *
     * @return
     */
    public static Date getNextWeekEnd() {
        Calendar cal = Calendar.getInstance();
        cal.setTime(getWeekEnd());
        cal.add(Calendar.DATE, 7);
        return cal.getTime();
    }

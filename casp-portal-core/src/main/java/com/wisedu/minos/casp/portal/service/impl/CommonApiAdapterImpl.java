...
public class CommonApiAdapterImpl implements CommonApiAdapter {
    ...
    public void sendMqMsg (String exchangeQueue, String content) {
        pubSubService.send(exchangeQueue,content);
    }
    ...
    /***
     * @Author jcx
     * @Description 获取门户License
     * @Date 19:54 2023/3/6
     * @return PortalLicenseDto
     **/
    public PortalLicenseDto getPortalLicense(){
        PortalLicenseDto portalLicenseDto = new PortalLicenseDto();
        //获取license信息
        LicenseInfo licenseInfo = MinosLicenseManager.getLicenseInfo();
        List<LicenseInfo.App> apps = licenseInfo.getApps();
        if(org.apache.commons.collections.CollectionUtils.isEmpty(apps)){
            //开发环境
            portalLicenseDto.setPortalPc(true);
            portalLicenseDto.setPortalH5(true);
        }else{
            AtomicBoolean isAllExpired = new AtomicBoolean(true);
            apps.forEach(app->{
                List<LicenseInfo.Module> modules = app.getModules();
                ...
                if("casp".equalsIgnoreCase(app.getId())){
                    modules.forEach(module -> {
                        ...
                        if(org.apache.commons.collections.CollectionUtils.isNotEmpty(functions)){
                            functions.forEach(fun->{
                                if(Global.CASP_PC.equalsIgnoreCase(fun.getName())){
                                    portalLicenseDto.setPortalPc(true);
                                }else if(Global.CASP_H5.equalsIgnoreCase(fun.getName())){
                                    portalLicenseDto.setPortalH5(true);
                                }
                            });
                        }
            ...
            if(isAllExpired.get()){
                portalLicenseDto.setPortalPc(false);
                portalLicenseDto.setPortalH5(false);
            }
        }
        return portalLicenseDto;
    }

 ...
 */
@Service
public class PageInfoService extends ServiceImpl<PageInfoMapper, PageInfoEntity> {
...
//            }
    ...
    public PageContext getPageContext(HttpServletRequest request) {
        return getPageContext(request, null);
    }
     ...
     */
    public PageContext getPageContext(HttpServletRequest request, String pageCode) {
        // del by zjchen   why?
        ...
        if (StringUtils.isEmpty(pageCode)) {
            ...
            if (pageInfo == null) {
                throw new PageNotFoundException(String.format("页面：%s 不存在", pageCode));
            }
            //获取当前菜单
            ...
            if (menuEntity == null) {
                ...
                currentMenu = ObjectUtil.copyProperties(menuEntity, new MenuBiz());
...
//                lv1Menu = ObjectUtil.copyProperties(parentMenu, new MenuBiz());
        ...
        return new PageContext().setShowProgrammeEntity(defaultProgramme)
                .setCurrentLv1Menu(lv1Menu).setCurrentMenu(currentMenu).setMenuTree(menus).setPageInfoEntity(pageInfo);
    }

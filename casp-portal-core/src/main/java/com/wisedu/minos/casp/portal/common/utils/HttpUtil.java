...
public class HttpUtil {
    ...
    public  static boolean isAjaxRequest(HttpServletRequest request){
        return StringUtils.isNotBlank(request.getHeader("x-requested-with")) && "XMLHttpRequest".equals(request.getHeader("x-requested-with"));
    }

    public static void setKeyValueInCookie(String key, String value, HttpServletResponse response){
        ...
        try {
            ...
            log.warn("对cookie的value进行URL编码异常", e);
        }
    ...
    public static  String getValueFromSessionAndCookie(String key,HttpServletRequest request){
        String siteWid = (String) request.getSession().getAttribute(Global.SITE_WID);
        if(StringUtils.isNotBlank(siteWid)){
            return siteWid;
        }
        Cookie[] cookies = request.getCookies();
        if(cookies!=null &&cookies.length>0){
           return   Stream.of(cookies).filter(e->key.equals(e.getName())).findAny().orElseGet(()->new Cookie(key,"")).getValue();
        }
        return null;
    }

...
//    }
    ...
    public List<TemplateContent> getTemplateInfo(String id,int platformType){
        ...
        List<TemplateContent> templateContents = new ArrayList<>();
        //判断是否移动端，读取的配置文件位置不同
        // 目录穿越 问题修改 zhangjian 2021-12-02
        if (caspSafetyService.legalCharacter(id)){
            logger.error("获取"+id+"不合法，不允许获取模板");
            return templateContents;
        }
...
//        this.processHeaderSubTitle(pageConfigMap);
        this.process(pageConfigMap,"header.title");
        this.process(pageConfigMap,"header.subTitle");
        this.process(pageConfigMap,"search.searchPrompt");
    ...
    protected void process(Map<String, Object> pageConfigMap,String key) {
        if(key.contains(".")){
            List<LangResoure> langs = new ArrayList<>();
            pageConfigMap.forEach((s, o) -> {
                if (s.startsWith(key)) {
                    String keyStr=key.substring(0,key.indexOf("."));
                    String keySed=key.substring(key.indexOf(".")+1);
                    langs.add(new LangResoure().setKey(s.replaceAll(""+keyStr+"\\."+keySed+"\\.", "")).setValue((String) o));
                }
            });
            pageConfigMap.put(key+"Langs", langs);
        }
    }
    ...
    protected String getRandomService() {
        logger.info("获取随机服务");
        AllServiceRequest allServiceRequest = new AllServiceRequest().setPageSize(Global.MAXSIZE);
    ...
    protected String getRandomServiceItem(String lang) {
        ...
        List<AllServiceItemResponse.ServiceItemModel> allServiceItem = serviceItemApiService.getAllServiceItem
                (new AllServiceItemRequest().setService(false).setIsRelate(0).setUserId(userId).setUserAccount(userAccount).setLang(lang));

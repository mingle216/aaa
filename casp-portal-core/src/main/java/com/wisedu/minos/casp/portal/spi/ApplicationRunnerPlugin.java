    ...
    public void run(ApplicationArguments args){
        ...
        if(StringUtil.isEmpty(editLangEnUsToEnFlag)){
            ...
            if(CollectionUtils.isNotEmpty(pageInfos)){
                ...
                systemConfigService.save(newSystemConfigEntity("", "editLangEnUsToEnFlag", "editLangEnUsToEnFlag", "", "3.2.1.beta2版本页面配置中英文多语言为en_US改为en", 2));
            }
        }
        ...
        if(StringUtil.isEmpty(official_default_pro_flag)){
            ...
            systemConfigService.save(newSystemConfigEntity("", "official_default_pro_flag", "official_default_pro_flag", "", "3.2.1.beta2修改official默认展示方案布局卡片地址", 2));
        }
        ...
        if(StringUtil.isEmpty(compensationCheck)){
            if(null==getDataBase(Global.LiuquibaseCompensate.UPDATE_20201219_DATA.getFileName())){
                List<CardEntity> list = cardService.list(new QueryWrapper<CardEntity>().in(DbFieldConstant.WID, Arrays.asList("3", "9")));
            ...
            List<DatabasechangelogEntity> list = databaseService.list(new QueryWrapper<DatabasechangelogEntity>()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210119_DATA.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210122_PAGE.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210401_PAGE_INFO.getFileName())
            );
            ...
            List<DatabasechangelogEntity> listSed = databaseService.list(new QueryWrapper<DatabasechangelogEntity>()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_PAGE_INFO0602_MOBILE.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_MOBILE_PAGE_INFO_LINK.getFileName())
            );
            ...
            List<DatabasechangelogEntity> listSun = databaseService.list(new QueryWrapper<DatabasechangelogEntity>()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210121_DATA.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210207_DATA.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210222_TEMPLATECONFIG.getFileName()).or()
                    .like(DbFieldConstant.FILE_NAME, Global.LiuquibaseCompensate.UPDATE_20210302_TEMPLATECONFIG.getFileName())
            );
...
//            }
            ...
            systemConfigService.save(newSystemConfigEntity("", "compensationCheck", "compensationCheck", "", "3.2.1.beta1去除之前版本所有sql脚本标志位", 2));
        }
        logger.info("casp-portal项目启动完毕........");
    }

    private DatabasechangelogEntity getDataBase(String liquiBaseFileName){
        return databaseService.getOne(new QueryWrapper<DatabasechangelogEntity>().like(DbFieldConstant.FILE_NAME,liquiBaseFileName));
    }
    ...
    private void addNewCardFir(){
        ...
        SystemConfigEntity systemConfigFir = newSystemConfigEntity("61", "mailSendChannelId", "", "", "消息通道id", 1);
        SystemConfigEntity systemConfigSed = newSystemConfigEntity("62", "mailSendInstanceWid", "", "", "消息插件id", 1);
        SystemConfigEntity systemConfigWed = newSystemConfigEntity("58", "coreMail_host", "", "121.248.60.50", "coremail域名", 1);
        SystemConfigEntity systemConfigFou = newSystemConfigEntity("59", "coreMail_port", "", "6195", "coremail端口", 1);
        SystemConfigEntity systemConfigFri =newSystemConfigEntity("60","coreMail_loginUrl_prefix","","http://121.248.60.50/coremail/main.jsp?","coremail登录地址前缀",1);
        SystemConfigEntity systemConfigQed =newSystemConfigEntity("56","academic_report","","http://yyzx.seu.edu.cn/southeast-university/academic-report/list?_=1617698262182","学术报告地址",1);
        SystemConfigEntity systemConfigRed =newSystemConfigEntity("57","library_lecture","","http://yyzx.seu.edu.cn/southeast-university/library-lecture/list/1?_=1617698310925","学生活动地址",1);
        SystemConfigEntity systemConfigYed = newSystemConfigEntity("55", "tencent_newCount_clientSecret", "", "", "未读邮件密钥", 2);
        ...
        if(CollectionUtils.isNotEmpty(list)){
            list.forEach(systemConfig -> {
                switch (systemConfig.getWid()){
                    case "50":
                        systemConfig.setConfigValue("tencent_accessTokenUrl");
                        break;
                    case "51":
                        systemConfig.setConfigValue("tencent_loginUrl");
                        break;
                    case "52":
                        systemConfig.setConfigValue("tencent_newCountUrl");
                        break;
                    case "53":
                        systemConfig.setConfigValue("tencent_clientId");
                        break;
                    default:
                        systemConfig.setConfigValue("tencent_login_clientSecret");
    ...
    private void addSysConfig(){
        SystemConfigEntity systemConfigFir= newSystemConfigEntity("50", "tencent_new_accessTokenUrl", "https://api.exmail.qq.com/cgi-bin/gettoken", "https://api.exmail.qq.com/cgi-bin/gettoken", "获取access_token接口地址", 2);
        SystemConfigEntity systemConfigSed= newSystemConfigEntity("51", "tencent_new_loginUrl", "https://api.exmail.qq.com/cgi-bin/service/get_login_url", "https://api.exmail.qq.com/cgi-bin/service/get_login_url", "获取登录企业邮箱的地址", 2);
        SystemConfigEntity systemConfigWed= newSystemConfigEntity("52", "tencent_new_newCountUrl", "https://api.exmail.qq.com/cgi-bin/mail/newcount", "https://api.exmail.qq.com/cgi-bin/mail/newcount", "获取未读邮件数量", 2);
        SystemConfigEntity systemConfigFou= newSystemConfigEntity("53", "tencent.new.clientId", "wma1cf2f625b012e46", "wma1cf2f625b012e46", "企业id", 2);
        SystemConfigEntity systemConfigFri= newSystemConfigEntity("54", "tencent.new.clientSecret", "VT8e16nasnKDn0SRoDHN16m_wQcDpDcIFFOgm-OXkiw", "VT8e16nasnKDn0SRoDHN16m_wQcDpDcIFFOgm-OXkiw", "应用的凭证密钥", 2);
    ...
    private SystemConfigEntity newSystemConfigEntity(String wid,String configKey,String configValue,
                                                     String defaultValue,String configDesc,Integer configModel){

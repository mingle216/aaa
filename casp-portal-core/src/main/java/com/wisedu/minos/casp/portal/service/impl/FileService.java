...
public class FileService {
    ...
    public ResultData uploadFile(MultipartFile file, String type) {
        ...
        try {
            if (StringUtil.isNotEmpty(filePath)) {
                ...
                cardTemUpVo.setCopyFilePath(attachInfoEntity.getReUrl());

                if (!"normal".equals(checkResult)) {
                    //存在相同卡片或者模板，返回给前台确认是否覆盖，如果覆盖，则提前创建副本，便于后续操作，因为finally会最终把源文件删除
                    cardTemUpVo.setNewCardTem(false);
                    return ResultData.success(ResultData.DEFAULT_SUCCESS_CODE, checkResult, cardTemUpVo);
                }
        ...
        try {
            if (null != cardTemUpVo.getIsCancel() && Global.CONSTANT_NO.equals(cardTemUpVo.getIsCancel())) {
                ...
                if (StringUtil.isNotEmpty(cardTemUpVo.getCopyFilePath())) {
                    ...
                    FileUtil.downloadFileFromURL(cardTemUpVo.getCopyFilePath(),cardTemUpVo.getUpLoadFilePath()+"/",newFileName);
     ...
     * @Param source:
     **/
    private String checkZip(File source, String path, CardTemUpVo cardTemUpVo) {
        String result = "";
...
//            boolean isHaveJar = false;
            ...
            if (StringUtil.isNotEmpty(dbVersionNumber)) {
                ...
                result = "当前" + uploadTypeName + "包中的" + uploadTypeName + "已安装&" + dbVersionNumber + "," + versionNumber + "";
                return result;
            }
        } catch (Exception e) {
...
//            }
     ...
     * @Param file:
     **/
    public AttachInfoEntity uploadFileToOss(MultipartFile file) {
        ...
        try {
            ...
            if (!file.isEmpty()) {
                ...
                if (upload) {
                    ...
                    //fileUrl保存相对路径
                    return saveAttachInfo(file.getOriginalFilename(), fileUrl, fileUrl.substring(userIconIndex, fileUrl.length()), fileLocation.toString());
                }
            }
        } catch (Exception e) {
     ...
     * @Param location:
     **/
    public AttachInfoEntity saveAttachInfo(String fileName, String fileRelUrl, String fileUrl, String location) {
        AttachInfoEntity attach = new AttachInfoEntity();
    ...
    public void cardTemplateHandler(File file, CardTemUpVo cardTemUpVo) {
        ...
        try {
            ...
            logger.info(uploadTypeName + "包入库失败！----{}", e);
...
//        String versionNumber = !"".equals(prop.getProperty("versionNumber"))||null==prop.getProperty("versionNumber")?prop.getProperty("versionNumber")+"_"+currentYearMonthDayTime:"1.0.0_"+currentYearMonthDayTime;
        ...
        if (uploadType.equals(Global.CARD_UPLOAD_TYPE)) {
            //上传的是卡片包
            if (proper) {
                throw new BusinessException("描述文件需要包含卡片ID、卡片名称、适配终端、是否支持实例级配置、版本号!");
            }
            if (StringUtils.contains(platformType, "0") && StringUtils.isBlank(imageUrl)) {
                throw new BusinessException("pc平台必须包含pc截图地址");
            }
            if (StringUtils.contains(platformType, "1") && StringUtils.isBlank(imageUrlMobile)) {
            ...
            cardEntity.setSchoolId(schoolId);
            ...
            templateEntity.setSchoolId(schoolId);
            return templateEntity;
        }
    }
    ...
    public String uploadLocalServer(MultipartFile file, String type) {
        if (StringUtil.isNotEmpty(type) && type.equals(Global.uploadLocalType.ICON.getType())) {
            //上传图标
            AttachInfoEntity attachInfoEntity = uploadFileToOss(file);
            //上传oss成功

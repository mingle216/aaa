...
public class FileUtil {
     ...
     * @Param file:
     **/
    public static File multipartFileToFile (MultipartFile file,String filePath) {

        File toFile = null;
        if ( file != null && file.getSize() > 0 && StringUtils.isNotBlank(filePath)
                && StringUtils.isNotBlank(file.getOriginalFilename())) {
            // 安全扫描发现此处存在 “任意文件写入” 问题，不能使用外部传入的文件名作为临时的文件名
            ...
            toFile = new File(dir,newFileName);
            if (toFile.exists()){
                toFile.delete();
            }
    ...
    public static boolean modifyFileContent (String filePath, String oldStr, String newStr) {
        try ( RandomAccessFile raf = new RandomAccessFile(filePath, "rw");){
            ...
            while ((line = raf.readLine()) != null) {
                oldText.append(line).append(lineSeparator);
            }
            String newText = oldText.toString().replace(oldStr, newStr);
            // 写入文件

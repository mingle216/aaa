...
import com.wisedu.minos.api.model.DubboAppService;
import com.wisedu.minos.api.model.DubboUserServiceReq;
import com.wisedu.minos.api.model.DubboUserServiceResp;
...
public class ServiceItemApiService {
    ...
    public List<AllServiceItemResponse.ServiceItemModel> getAllServiceItem(AllServiceItemRequest allServiceItemRequest) {
        ...
        allServiceItemRequest.setPageSize(2000);
        boolean isConditionSearch = StringUtil.isNotEmpty(allServiceItemRequest.getSearchKey()) || StringUtil.isNotEmpty(allServiceItemRequest.getCategoryWids()) || StringUtil.isNotEmpty(allServiceItemRequest.getDeptWids()) || StringUtil.isNotEmpty(allServiceItemRequest.getRoleWids()) || StringUtil.isNotEmpty(allServiceItemRequest.getDimensions())||StringUtil.isNotEmpty(allServiceItemRequest.getGroupWid());
        ...
        String key = "allServiceItem:userId:" + userId;
    ...
    public AllServiceItemResponse getServiceItemModels(AllServiceItemRequest allServiceItemRequest) {
        try {
            ...
            AllServiceItemResponse allServiceResponse = RestTemplateUtils.postForObject("/casp-sim/itemManager/queryItemList", httpEntity, AllServiceItemResponse.class);
    ...
    public BaseResponse visitServiceItem(VisitServiceItemRequest serviceRequest) {
        try {
            ...
            LOGGER.info("访问服务事项计数接口请求：" + JSON.toJSONString(serviceRequest));
            BaseResponse baseResponse = RestTemplateUtils.postForObject("/casp-sim/itemManager/visitItem", httpEntity, BaseResponse.class);
            LOGGER.info("访问服务事项计数接口返回：" + JSON.toJSONString(baseResponse));
            return baseResponse;
        } catch (Exception e) {
    ...
    public ServiceRelServiceResponse.ServiceItemAttr getServiceItemRelService(ServiceItemRelServiceRequest serviceRequest) {
        ...
        HttpEntity<ServiceItemRelServiceRequest> httpEntity = new HttpEntity<>(serviceRequest);
        LOGGER.info("调用服务事项详情接口请求：" + JSON.toJSONString(serviceRequest));
        ServiceItemDetailResponse serviceItemDetailResponse = RestTemplateUtils.postForObject("/casp-sim/serviceItem/selectById",
                httpEntity, ServiceItemDetailResponse.class);
        LOGGER.info("调用服务事项详情接口返回：" + JSON.toJSONString(serviceItemDetailResponse));
        if ("0".equals(serviceItemDetailResponse.getErrcode())) {
            ServiceItemDetailResponse.ServiceItemModel serviceItemModel = serviceItemDetailResponse.getData();
            if (serviceItemModel == null) {
                return response;
            }
            response.setIsShow(serviceItemModel.getIsShow());
            response.setItemName(serviceItemModel.getItemName());
            response.setIsEnabled(serviceItemModel.getIsEnabled());

            List<String> favoriteList= Lists.newArrayList();
            UserInfo user = ApplicationContextUtil.get(UserUtil.class).getCurrentUser();
            if (null != user && !StringUtil.isEmpty(user.getWid())) {
                DubboUserServiceReq req=new DubboUserServiceReq();
                req.setUserId(user.getWid());
                DubboUserServiceResp resp= favoriteService.userServiceFavoriteList(req);
                LOGGER.info("++++++++++++++favoriteService.userServiceFavoriteList:"+JSON.toJSONString(resp));
                if(CollectionUtils.isNotEmpty(resp.getData())){
                    favoriteList=resp.getData().stream().map(DubboAppService::getServiceWid).collect(
                            Collectors.toList());
                }
            }

            List<String> linkService = serviceItemModel.getLinkServices();
            if (CollectionUtils.isNotEmpty(linkService)) {
                ServicesRequest servicesRequest = new ServicesRequest();
                servicesRequest.setData(linkService);
                HttpEntity<List<String>> servicesRequestHttpEntity = new HttpEntity<>(linkService);
                LOGGER.info("调用查询服务接口请求：" + JSON.toJSONString(servicesRequestHttpEntity));
                ItemServiceLinkResponse itemServiceLinkResponse = RestTemplateUtils.post("/service/queryByWids", servicesRequest,
                        ItemServiceLinkResponse.class).getBody();
                LOGGER.info("调用查询服务接口返回：" + JSON.toJSONString(itemServiceLinkResponse));
                if ("0".equals(itemServiceLinkResponse.getErrcode()) && itemServiceLinkResponse.getData() != null) {
                    serviceModelList = itemServiceLinkResponse.getData();
                    //按端筛选展示服务
                    getServiceListByPlatform(serviceModelList, linkService, response,favoriteList);
                    handlerLangCountry(response.getServiceList(),serviceRequest.getLangCountry());
                }
            }
        } else {
            throw new BusinessException("查询服务事项详情失败:" + serviceItemDetailResponse.getErrcode() + serviceItemDetailResponse.getErrmsg());
        }
        return response;
    }

    private void getServiceListByPlatform(List<ServiceRelServiceResponse.ServiceModel> serviceModelList, List<String> linkService, ServiceRelServiceResponse.ServiceItemAttr response,List<String> favoriteList){
        if (CollectionUtils.isNotEmpty(serviceModelList)) {
            ...
            for (String wid : linkService) {
                ...
                // 按端展示
                handleService(serviceModelList, canUseServiceWids, canVisitedServiceWids, serviceModel,favoriteList);
            }
            response.setServiceList(serviceModelList);
        }
    }

    private void handleService(List<ServiceRelServiceResponse.ServiceModel> serviceModelList, List<String> canUseServiceWids, List<String> canVisitedServiceWids, ServiceRelServiceResponse.ServiceModel serviceModel,List<String> favoriteList) {
        if (serviceModel != null) {
            ...
            if(CollectionUtils.isNotEmpty(favoriteList)&&favoriteList.contains(serviceModel.getWid())){
                serviceModel.setFavorite(1);
            }
    ...
    public QueryAllServiceItemResponse queryPageAllServiceItem(String... parms) {
        ...
        try {
            response = RestTemplateUtils.post("/casp-sim/itemManager/getQueryItemPageList", entity, QueryAllServiceItemResponse.class).getBody();
    ...
    public QueryItemPageListForCaspRes getQueryItemPageListForCasp(String markId,String pageSize) {
        ...
        LOGGER.info("调用获取全部服务事项(,usr=" + queryAllServiceItemRequest + "参数=" + JSON.toJSONString(entity));
        try {
            response = RestTemplateUtils.post("/casp-sim/itemManager/getQueryItemPageListForCasp", entity, QueryItemPageListForCaspRes.class).getBody();
            LOGGER.info("调用获取全部服务事项接口成功，返回结果：" + JSON.toJSONString(response));
        } catch (Exception e) {

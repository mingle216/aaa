...
@Service
public class SystemConfigService extends MyServiceImpl<SystemConfigMapper, SystemConfigEntity> {
    @Autowired
    ...
    public String getSystemConfigValue(String configKey) {
        ...
        if (!CollectionUtils.isEmpty(systemConfigList)) {
            ...
            return org.apache.commons.lang3.StringUtils.isBlank(systemConfig.getConfigValue()) ? systemConfig.getDefaultValue() : systemConfig.getConfigValue();
        }
        return null;
    }
    ...
    public SystemConfigEntity getSystemConfigForPortal(String configKey) {
        List<SystemConfigEntity> systemConfigList = systemConfigMapper.selectList(new QueryWrapper<SystemConfigEntity>().lambda()
                .select(SystemConfigEntity::getConfigValue,SystemConfigEntity::getDefaultValue).eq(SystemConfigEntity::getConfigKey, configKey).eq(SystemConfigEntity::getIsDeleted, 0));
        if (!CollectionUtils.isEmpty(systemConfigList)) {
            SystemConfigEntity systemConfig = systemConfigList.get(0);
            return systemConfig;
        }
    ...
    public Map<String,String> getSystemConfigsForMap(List<String> configKeys) {
        List<SystemConfigEntity> systemConfigEntities =  systemConfigMapper.selectList(Wrappers.<SystemConfigEntity>lambdaQuery()
                .eq(SystemConfigEntity::getIsDeleted, Global.DeleteStatus.NO.getId())
                .in(SystemConfigEntity::getConfigKey, configKeys));

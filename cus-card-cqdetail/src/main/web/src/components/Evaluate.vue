<template>
  ...
  >
    <AutoContainer :conMaxHeight="438">
      <div style="padding: 20px 20px 20px 20px;">
        ...
        <div class="common-flex" style="margin-top:16px">
          <div style="margin-right: 4px;">{{ $Lan( lanFunName, "uploadImg", "上传图片（选填）" ) + '：' }}</div>
          <div class="portal-font-color-lv3">{{ $Lan( lanFunName, "evaluateUploadTip", "可上传png或jpg格式图片，单张大小不超过500k，最多上传3张" ) }}</div>
        </div>
        <!-- 图片预览 -->
        <div class="uploadPicLists">
          ...
          <div v-if="uploadLists.length > 0" class="upB">
            <div class="uploadPic" v-for="(item, index) in uploadLists" :key="index">
              ...
              <div class="uploadPic-hover">
                <i class="we-icon-delete" @click="handleDeletePic(index)"></i>
              </div>
            </div>
          </div>
          <!-- 上传按钮 -->
          ...
          >
            <div class="upBtnBox">
              <i class="we-icon-plus plusIcon"></i>
            </div>
          </we-upload>
        </div>
      </div>
...
<style lang="less">
.upBtnBox{
  width: 90px; height: 90px; border-radius: 4px; cursor: pointer; border: 1px dashed #D9D9D9; background-color: #FAFAFA; display: flex; justify-content: center; align-items: center;
  .plusIcon{
    font-size: 20px;
  }
}

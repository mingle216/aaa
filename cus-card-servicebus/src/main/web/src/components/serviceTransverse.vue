<template>
  ...
  <div ref="father" class="serviceTransverse-father">
    ...
    <div class="serviceTransverse" v-else>
      <header
        v-if="allClassifyData.length > 1"
        :style="`${foldFlag ? 'height: 68px;' : 'height: auto;'}`"
      >
        <div class="serviceTransverse-tablist" ref="containter">
          <ul
            ref="tablist"
            :style="foldFlag ? 'white-space: nowrap;' : 'white-space: wrap;'"
          >
            <li
              ...
              :class="{
                ...
                'color-bold': selectedTypeId === item.typeId,
          ...
          <div class="tab-pn" v-if="tabConfigure.isShow">
            ...
            <span
              ...
              :class="{
                'pn-disabled':
                  Math.abs(tabConfigure.translateX) === tabConfigure.maxDistance,
              }"
            >
              <i class="icon-dongdamoban-youjiantou iconfont"></i>
            </span> -->
            <span v-if="foldFlag" class="fold-btn portal-primary-color-lv1" @click="foldFlag = false">{{
              $Lan(LanFunName, "expand", "展开")
            }}</span>
            <span v-else class="fold-btn portal-primary-color-lv1" @click="foldFlag = true">{{
              $Lan(LanFunName, "fold", "收起")
            }}</span>
          </div>
        </div>
        <span
          v-if="allClassifyData.length > 1 && userInfo"
          @click="handleSubscribe"
          class="serviceTransverse-setting portal-primary-color-lv1"
          ...
          <template v-else>
            <ul v-for="(services, pIndex) in servicesData" :key="pIndex">
              <li v-for="(item, index) in services" :key="index" :style="itemWidth">
                <div
                  class="service-list portal-primary-backgroundcolor-hover-lv5 portal-primary-border-color-lv4"
                ...
                >
                  ...
                  <div style="width: 0; flex: 1; word-wrap: break-word">
                    <we-tooltip
                      effect="dark"
...
export default {
  ...
  data() {
    return {
      LanFunName: "CUS_CARD_SERVICEBUS",
      selectedTypeId: "",
  ...
  methods: {
    ...
    next() {
      let { sonWidth, fatherWidth, maxDistance, translateX } = this.tabConfigure;

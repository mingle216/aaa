<template>
  ...
  >
    <div class="modal_title" v-if="showTitle">
      <span>
        {{ $Lan('CUS_TEMPLATE_OFFICIAL_h5', 'onlineService', '在线办理') }}
      </span>
    ...
    <div class="modal__wrap pc_service" v-else>
      ...
      <p class="pc_tips_top clamp">
        {{ $Lan('CUS_TEMPLATE_OFFICIAL_h5', 'pcService', '该服务为PC服务') }}
      </p>
      <p :class="{'pc_tips_bottom': isPcServiceSupported, 'pc_tips_text': !isPcServiceSupported, 'clamp': 1}">
				{{ tooltipText }}
      </p>
      <we-button
				v-if="isPcServiceSupported"
        class="open_btn portal-primary-backgroundcolor-lv1 portal-primary-border-color-lv1"
        type="info"
        @click.native="handlePcOk"
        style="width:100%"
      >
        {{ $Lan('CUS_TEMPLATE_OFFICIAL_h5', 'open', '打开') }}
      </we-button>
    </div>
  </we-popup>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      pcOpen: false,
      showTitle: true,
			isPcServiceSupported: 0
    };
  },
	computed: {
		tooltipText() {
			return this.isPcServiceSupported ? this.$Lan('CUS_TEMPLATE_OFFICIAL_h5', 'openInMobile', '是否在移动端打开?') :
				this.$Lan('CUS_TEMPLATE_OFFICIAL_h5', 'openInPC', '请前往PC端访问')
		}
	},
	watch:{
		pcServiceType(val){
			this.isPcServiceSupported = val
		}
	},
	methods: {
    getIconUrl(item) {

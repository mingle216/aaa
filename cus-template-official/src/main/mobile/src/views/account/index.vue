<template>
  <div class="account-manage">
    <div class="head">
      ...
      />
      <span class="title">
        {{ $Lan("CUS_TEMPLATE_OFFICIAL_h5", "accountManage", "帐号管理") }}
      </span>
    </div>
    ...
    <div class="content">
      <div class="item" @click="selectLanguage">
        <span>
          {{ $Lan("CUS_TEMPLATE_OFFICIAL_h5", "Language", "语言") }}
        </span>
      ...
      <!-- <div class="item" @click="changeSites" v-if="siteList.length > 1">
        <span>
          {{ $Lan("CUS_TEMPLATE_OFFICIAL_h5", "changeSites", "切换站点") }}
        </span>
        <div class="pos-right">
      ...
      <div class="item" @click="logout" style="margin-top: 16px">
        {{ $Lan("CUS_TEMPLATE_OFFICIAL_h5", "logout", "退出登录") }}
      </div>
    </div>
    <we-action-sheet
    ...
    supportLanguages() {
      ...
      if (lanarr.length > 0) {
        lanarr.forEach((tem) => {
          ...
          tem.className = this.$i18n.locale === tem.langCode ? 'portal-primary-color-lv1' : ''
        });
      }

<template>
  ...
  >
    <div class="mobileAppPopover__content portal-font-color-lv1">
      ...
      <div class="word">
        <p>
          {{ $Lan('SYS_TEMPLATE_OFFICIAL', 'mobileService', '此服务为移动端服务') }}
        </p>
        <p>
          {{ $Lan('SYS_TEMPLATE_OFFICIAL', 'scanCode', '请手机扫二维码打开') }}
        </p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    show(targetEle) {
      ...
      let classList = [...this.targetEle.classList]
      this.flag = classList.includes("service__item")
      // console.log(this.targetEle)
      // this.flag=this.targetEle.classList.value.indexOf('service__item')!==-1?true:false // 有图标显示的二维码框
      this.isShow = true;
      this.setPlacement();
    },
    setPlacement() {
      ...
      if (this.placement === "bottom") {
        let curLeft = this.flag && !this.isOneMobile
          ? `${left + 10}px`
    ...
    handleMobileOk() {
      window.shell.openService(this.itemInfo, null, true);
      this.handleMobileclose();
    },
    handleScroll() {
      if (!this.isShow) {

<template>
  <div>
    <we-dialog
      :title="$Lan('SYS_TEMPLATE_OFFICIAL', 'changeSites', '切换站点')"
    ...
    >
      <vue-scroll :barKeepShow="true">
        <ul class="modal__wrap">
          ...
          >
            ...
            >
              {{ $Lan("SYS_TEMPLATE_OFFICIAL", "using", "使用中") }}
            </span>
          </li>
        </ul>
      </vue-scroll>
      <div slot="footer" class="dialog__footer">
        <we-button size="medium" @click="close">
          {{ $Lan("SYS_TEMPLATE_OFFICIAL", "cancel", "取消") }}
        </we-button>
        <we-button
          size="medium"
          type="primary"
        ...
        >
          {{ $Lan("SYS_TEMPLATE_OFFICIAL", "confirmChoice", "确认选择") }}
        </we-button>
      </div>
    </we-dialog>
  </div>

<template>
  ...
  >
    <div v-if="isExpand" class="side_box">
      <div
        class="expand__side"
        v-for="(el, i) in Math.ceil(sideList.length / 5)"
        :key="i"
      >
        <div
      ...
      >
        ...
        <span class="desc text__ellipsis">
          {{ $Lan("SYS_TEMPLATE_OFFICIAL", "fold", "收起") }}
        </span>
      </div>
    </div>
    <div
    ...
    >
      ...
      >
        ...
        ></i>
        <span class="desc text__ellipsis">
          {{ $Lan("SYS_TEMPLATE_OFFICIAL", "showAll", "查看全部") }}
        </span>
      </div>
    </div>
    <div
...
export default {
  ...
  computed: {
    transformHeight() {
      const height =
        (this.sideList.length > 5 ? 5 : this.sideList.length) * this.itemHeight;
    ...
    showPopover(dom, i, dir) {
      this.$nextTick(() => {
        ...
        if (dir == "right") {
          ...
          d_left =
            Number(left) + 66 * (Math.ceil(this.sideList.length / 5) - (i + 1));
        }

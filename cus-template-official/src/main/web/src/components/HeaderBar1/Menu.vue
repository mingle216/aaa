  ...
  <we-header class="home__menu">
    ...
    >
      <NavMenu
        ...
        :navMenus="getMenu.length > 6 ? getMenu.slice(0, 5) : getMenu"
        :activeIndex="activeIndex"
        :dom="mentDom"
      />
      <el-submenu
        v-if="getMenu.length > 6"
        index="more"
      ...
      >
          <template slot="title">
            ...
            <span class="more__title">
              {{ $Lan('SYS_TEMPLATE_OFFICIAL', 'more', '更多') }}
            </span>
          </template>
...
export default {
  ...
  computed: {
    ...
    moreList() {
      return this.getMenu.slice(5, this.getMenu.length)
    },
    downArrow() {

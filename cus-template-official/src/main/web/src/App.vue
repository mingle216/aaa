<template>
  <div id="app" ref="appCon">
    ...
    <sites-modal ref="SitesModal" />
  </div>
</template>

<script>
import Home from "./views/home";
import Dzhome from "./views/dzhome";
...
export default {
  ...
  computed: {
    ...
    showWise() {
      const config =
        this.pageData &&
        this.pageData.showProgrammeEntity &&
        this.pageData.showProgrammeEntity.templateConfig;
      const temp = JSON.parse(JSON.parse(config));
      return temp?.showWise == "1";
    },
    wiseLink() {
      const config =
        this.pageData &&
  ...
  watch: {
    showWise(val) {
      if (val) {
        document.querySelector(".index_robot__3jJNy").style.display = "block";
      } else {
        document.querySelector(".index_robot__3jJNy").style.display = "none";
      }
    },
  },
  async created() {
    ...
    window.shell.on("open-sites-modal", (sites) => {
      this.$refs.SitesModal.show(sites);
    });
  ...
  beforeDestroy() {
    ...
    window.shell.off("open-sites-modal");
  },
  methods: {
    ...
    goWiseUrl() {
      const url = this.wiseLink[0].langValue || "";

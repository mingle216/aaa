...
export const getI18nSettings = () => {
  return {
    ...
    dayNamesShort: weeks.map(week => locale === 'zh_CN' ? defaultWeek[week] : enWeek[week]),
    dayNames: weeks.map(week => locale === 'zh_CN' ? defaultWeek[week] : enWeek[week]),
    monthNamesShort: months.map((index) => window.shell.getLanguageValue("SYS_CARD_CALENDAR", `monthShort${index}`, `${ index + 1 }月`)),
    monthNames: months.map((index) => window.shell.getLanguageValue("SYS_CARD_CALENDAR", `month${index}`, `${ index + 1 }月`)),
    amPm: ['am', 'pm']
  };
};
...
// 获取当前为周几
export const getWeekDay = function(src) {
  if (!isDate(src)) return null;
  const week = weeks[new Date(src).getDay()];
  return locale === "zh_CN" ? defaultWeek[week] : enWeek[week];
};

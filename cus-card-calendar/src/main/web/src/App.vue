<template>
  <div class="cardCalendar portal-font-color-lv1" v-loading="loading">
    <!-- 标题 -->
    ...
    <ContainerScroll :scrollingX="true" :barKeepShow="true">
      ...
      <calendar v-model="selectedDate" ref="Calendar">
        ...
        <template slot="dateCell" slot-scope="{ text, data }">
          <div class="cardCalendar__date">
            ...
            >
            <!-- 休 -->
            <span class="cardCalendar__holiday" v-if="getDayType(data.day)">{{
              getDayType(data.day) == 1 ? "节" : "休"
            }}</span>
          </div>
        </template>
      </calendar>
      ...
      >
        ...
        <div v-else class="cardCalendar__empty ellipsis portal-font-color-lv4">
          {{ $Lan("CUS_CARD_CALENDAR", "noDaycircle", "暂无日程") }}
        </div>
        <div
...
import { formatDate, getFirstDayOfMonth, getPrevMonthLastDays } from "./utils/date-util";
...
export default {
  ...
  computed: {
    ...
    conHeight() {
      ...
      value = value - (this.router.showTitle ? 8 : 0);
      return this.containerType == 0 ? value : value - (this.showHeader ? 54 : 0) - 320; //减掉标题和日历的高度
    },
  },
  watch: {
  ...
  methods: {
    getDayType(day) {
      const schedule = this.dataListsObj[day];
      if (schedule?.length) {
        let dayType = false;
        for (let index = 0; index < schedule.length; index++) {
          const ele = schedule[index];
          if (ele.dayType == 1 || ele.dayType == 2) {
            dayType = ele.dayType;
            if (dayType == 1) {
              break;
            }

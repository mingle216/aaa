package com.wisedu.amp.card.cus.calendar.plugin;
...
import com.wisedu.amp.card.cus.calendar.dto.CalendarDtoV2;
import com.wisedu.amp.card.cus.calendar.enums.CalendarConsoleDto;
import com.wisedu.amp.card.cus.calendar.enums.QueryModelEnum;
...
public class CalendarCard extends AbstractCard {
    ...
    private Object pullDingdingData(CardAjaxRequest request) {
        ...
        if (currentUser == null) {
            throw new BusinessException("-1", "用户未登录不允许拉取配置");
        }
...
//                List<CalendarInfo> collect = res.stream().sorted(Comparator.comparing(CalendarInfo::getWid)).collect(Collectors.toList());
    ...
    private Object renderData(CardAjaxRequest request) {
        ...
        if ("0".equals(response.getErrcode())) {
            ...
            datas = data.stream().filter(e -> e.getCalWid().equals("1") || StringUtil.isNotBlank(selectMap.get(e.getCalWid()))).map(e -> {
    ...
    private List<JSONObject> getChannel() {
        ...
        Map<String, String> sendChannel = ApplicationContextUtil.get(SystemToolsService.class).getSendChannel();
        ...
        list.sort((o1, o2) -> {
            if (o1.get("value").equals("INBOX")) {
                return -1;
            } else if (o2.get("value").equals("INBOX")) {
                return 1;
            }
            return 0;
        });

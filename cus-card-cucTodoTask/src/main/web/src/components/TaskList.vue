<template>
  <div class="todotask-list">
      ...
      <div class="todotask-list-main" v-if="subList.length">
        <div v-for="(item, index) in subList" class="list-item" :key="item.appId + item.taskId + index">
          <template v-if="
            ...
            (item.priorityCode == 1 || item.priorityCode == 2)
          ">
            <we-tooltip :offset="14" class="tip" effect="dark" :content="
              item.priorityCode == 1
                ? $Lan(lanFunName, 'extraUrgent', '特急')
                : $Lan(lanFunName, 'urgent', '紧急')
            " placement="bottom-start" :open-delay="800">
              <div v-if="item.priorityCode" class="icon-con" :class="['icon' + item.priorityCode]">
          ...
          <div class="item-top" :class="{
            ...
            <div class="tast-title">
              <we-tooltip class="text-ellipsis" effect="dark" placement="bottom-start" :open-delay="800">
                <div slot="content">{{ item.subject }}</div>
                <div class="portal-font-color-lv1 portal-primary-color-hover-lv1" style="cursor:pointer"
                  @click="e => move(item, e)">
                  {{ item.subject }}
                </div>
              </we-tooltip>
              <div class="newTag" v-if="item.isNew == 1" />
              <we-tooltip class="text-ellipsis portal-font-color-lv1" effect="dark" v-show="item.isTop === '1'">
                <div slot="content">
            ...
            <!-- 收藏 -->
            <span v-if="isFavorite == '1'" class="iconfont icon-icon-mianxingshoucang favoriteIcon"
              :class="[item.isFavorite === '1' ? '' : 'unfavorite']" @click.stop="handleFavorite(item)"></span>
          </div>
          <div class="item-bottom portal-font-color-lv3" v-if="!noneShowItem || itemConfigure.includes('time')">
            ...
            <!--放开并注释下面一行-->
            <div class="item-bottom-left" :class="[showItemNum]" v-if="!noneShowItem">
              ...
              <we-tooltip v-if="itemConfigure.includes('scene') && taskSource === 2" class="show-item text-ellipsis"
                effect="dark" placement="bottom-start" :open-delay="800" :disabled="disabledToolTip(item, 'source_NAME')">
              ...
              <we-tooltip v-if="itemConfigure.includes('author')" class="show-item text-ellipsis" effect="dark"
                placement="bottom-start" :open-delay="800" :disabled="disabledToolTip(item, 'initiatorName')">
              ...
              <we-tooltip v-if="itemConfigure.includes('dept')" class="show-item text-ellipsis" effect="dark"
                placement="bottom-start" :open-delay="800" :disabled="disabledToolTip(item, 'initiatorDept')">
                <div slot="content">
                  {{ item.initiatorDept || "-" }}
                </div>
                <div class="dept">
            ...
            <!--发起时间-->
            <div class="creat-time text-ellipsis" :class="{
    ...
    <we-popover ref="MobilePop" popper-class="todoTaskPop" placement="bottom" width="175">
      <div class="MobilePop">
    ...
    <div class="pages" v-if="total > 0">
      ...
      <we-pagination background layout="prev, pager, next" :total="total" :page-size="pageSize" :pager-count="5"

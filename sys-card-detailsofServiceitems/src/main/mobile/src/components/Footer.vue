    ...
    >
      <div class="MatterDetail-content">
        <ul class="evaluate-form portal-font-color-lv1">
          ...
          <li class="evaluate-form-item evaluate-form-item1">
          ...
          <li class="evaluate-form-item" style="flex-wrap: wrap;">
            <div class="evaluate-form-item-label" style="width: 100%;">
              {{ $Lan(lanFunName, "uploadImg", "上传图片（选填）") }}：
            </div>
            <div class="tips">{{ $Lan(lanFunName, "tips", "可上传格式为.png、.jpg的图片，单张图片不超过500kb，最多上传3张") }}</div>
            <div class="uploadPicLists">
              <!-- 图片列表 -->
              <div v-if="uploadLists.length > 0" class="upB">
                <div class="uploadPic" v-for="(item, index) in uploadLists" :key="index">
                  <img :src="getImgUrl(item)" alt="" style="width: 100%; height: 100%; object-fit: cover;">
                  <div class="closeP" @click="handleDeletePic(index)">
                    <we-icon class="weClose" name="cross" />
                  </div>
                </div>
              </div>
              <!-- 上传按钮 -->
              <we-uploader
                :before-read="handleChangeUpload"
                :max-count="3"
                v-show="uploadLists.length < 3"
              >
                <div class="upBtnBox">
                  <we-icon name="plus" class="plusIcon" />
                </div>
              </we-uploader>
            </div>
          </li>
...
export default {
  ...
  methods: {
    ...
    async submitEvaluate() {
      ...
      this.$emit("submitEvaluate", {params: param, uploadLists: this.uploadLists});
    },
  },
  watch: {
    appraiseName: {
...
.upBtnBox{
  width: 2.666667rem; height: 2.666667rem; border-radius: 4px; cursor: pointer; border: 1px dashed #D9D9D9; background-color: #FAFAFA; display: flex; justify-content: center; align-items: center;
  .plusIcon{
    font-size: 20px;
  }
}
.uploadPic {
  ...
  overflow: hidden;
  .closeP{
    position: absolute; top: 0; right: 0; width: 0.533333rem; height: 0.533333rem; background-color: rgba(0, 0, 0, 0.7); display: flex; justify-content: center; align-items: center;
    .weClose{
      color: white; font-size: 0.426667rem;
    }
  }
}
.uploadPicLists {
...
.tips{
  font-size: 0.373333rem; color: #707D8F; line-height: 0.586667rem; margin-top: 0.213333rem;
}

<template>
  <div class="cards-wrap" v-loading="cardLoading">
          ...
          <div class="cards-app-item portal-primary-border-color-hover-lv1" v-for="app in item.data" :key="app.itemWid">
            <div class="app-top">
              ...
              <i
                ...
                v-if="currentUser"
                :class="app.favorite ? 'isFav' : ''"
                @click="collectItem(app)"
              ></i>
              <!--<i class="we-icon-star-off" v-if="currentUser" :class="app.favorite?'isFav':''" @click="collectItem(app)"></i>-->
            </div>
       
            <div class="app-bot"  style="justify-content: center;">
              <p
                class="portal-primary-backgroundcolor-hover-lv1"
                :class="app.onlineServiceType === 2 ? '' : 'unClick'"
                @click="handleOnline(app)"
                style="width: 100%;"
              >
                进入数字驾驶舱
              </p>
    ...
    <div class="cards-tab1" :style="navStyle">
      <p
        v-for="(item, index) in tabList"
        :class="tabIndex == index ? 'portal-primary-color-lv1 portal-primary-backgroundcolor-lv5' : ''"
...
<script>
export default {
  props: ['cardData', 'content_width', 'currentUser', 'serviceItemListLimit', 'showMore', 'cardLoading', 'router'],
  data() {
    return {
      scrollNum: 0,
      tempTop: 0,
    ...
    initNavScoll(topNum) {
      ...
      if (this.isFlags) {
        for (let i = 0; i < this.tabList.length; i++) {
          if (
            this.tabList[i + 1] &&
            document.getElementById(this.tabList[i]).getBoundingClientRect().top <= 72 &&
            document.getElementById(this.tabList[i + 1]).getBoundingClientRect().top > 72
          ) {
            this.tabIndex = i
            break
      ...
      if (indexNavTop + 72 >= this.startTop) {
        console.log(indexNavTop,this.startTop,'%%%%%%%%%%%%%%%');
        let con_el = document.getElementById('cusNav')

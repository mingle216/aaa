  ...
  >
    ...
    <di class="showError" v-if="showError">
      ...
      <div class="showError__flex">
        ...
        <span
          class="serviceName"
          :class="[isMobile ? '' : 'ellipsis']"
          :title="serviceName"
          >{{ serviceName }}</span
        >
      </div>
      <template v-if="serviceDesc">
        <div class="serviceDesc portal-font-color-lv2" v-if="isMobile">
          {{ serviceDesc }}
        </div>
        <v-clamp
          v-else
...
export default {
  ...
  async mounted() {
    // 先检测服务的url是否可访问，不可访问时展示异常信息;移动端在底座处理了该逻辑，直接展示错误异常信息
    if (this.isMobile) {
      this.showError = true;
      this.getServiceInfo();
    } else {
      const isValid = await globalObj.checkServiceHealth(

...
public class NewMobileAppInfoController {
    ...
    @ResponseBody
    @RequestMapping(value = "/userAppListGroupByCategoryNew", method = {RequestMethod.GET, RequestMethod.POST})
    public Map<String, Object> userAppListGroupByCategoryNew(HttpServletRequest request,
                                                          @RequestParam(value = "categoryId", defaultValue = "") String categoryId,
                                                          @RequestParam(value = "pageNumber", defaultValue = "1") int pageNumber,
                                                          @RequestParam(value = "pageSize", defaultValue = "16") int pageSize,
                                                          @RequestParam(value = "keyword", defaultValue = "") String keyword) {
        logger.debug("准备调用MobileAppInfoController.userAppListGroupByCategoryNew方法，传入参数  categoryId：{}; keyword: {}; pageNumber: {}; pageSize:{}",
                categoryId, keyword, pageNumber, pageSize);
        try {
            UserInfo user = ApplicationContextUtil.get(UserUtil.class).getCurrentUser();
            DubboAmpMobileServiceRequest ampMobileServiceRequest = new DubboAmpMobileServiceRequest();
            ...
            ampMobileServiceRequest.setRecommendServiceWids(getRecommendServiceList());
            DubboAmpMobileServiceResponse response = serviceAmpMobileService.userAppList(ampMobileServiceRequest);
            return toClientAppListGroupByCategoryNew(response.getData(), categoryId);
        } catch (Exception e) {
            logger.error("查询用户有权看到的应用列表出错，原因: ", e);
            return responseError(e.getMessage());
        }
    ...
    private Map<String, Object> toClientAppListGroupByCategoryNew(List<DubboAmpMobileServiceInfo> serviceInfoList, String categoryId) {
        List<Map<String, Object>> categoryList = new ArrayList<>();
        DubboAmpMobileTreeCategoryResponse categoryResponse = serviceAmpMobileService.getCategoryAndServiceTreeList(categoryId);
        List<DubboAmpMobileTreeCategoryInfo> categoryInfoList = categoryResponse.getData();
        ...
        Map<String, DubboAmpMobileServiceInfo> serviceMap = serviceInfoList.stream().collect(Collectors.toMap(DubboAmpMobileServiceInfo::getWid, Function.identity()));
        //所有服务wid，计算总数用
        List<String> setAppIds = new ArrayList<>();
        for(DubboAmpMobileTreeCategoryInfo categoryInfo : categoryInfoList){
            Map<String, Object> map = new HashMap<>();
            map.put("categoryId", categoryInfo.getWid());
            map.put("categoryName", categoryInfo.getName());
            // 处理service
            List<Map<String, Object>> serviceList = new ArrayList<>();
            List<String> recursionServiceWids = categoryInfo.getRecursionServiceWids();
            if(CollectionUtils.isNotEmpty(recursionServiceWids)){
                for(String serviceWid : recursionServiceWids){
                    DubboAmpMobileServiceInfo serviceInfo = serviceMap.get(serviceWid);
                    if(null != serviceInfo && (flag || serviceInfo.getHasPermission())){
                        setAppIds.add(serviceInfo.getWid());
                        Map<String, Object> objectMap = NewMobileClientDataAdapter.toClientAppData(serviceInfo);
                        if (objectMap != null) {
                            serviceList.add(objectMap);
                        }
                    }
                }
                if (CollectionUtils.isNotEmpty(serviceList)) {
                    map.put("apps", serviceList);
                }
            }

            List<Map<String, Object>> secondCategoryList = new ArrayList<>();
            map.put("children", secondCategoryList);
            categoryList.add(map);
            // 二级分类
            List<DubboAmpMobileTreeCategoryInfo> secondCategoryInfoList = categoryInfo.getChildren();
            if(CollectionUtils.isNotEmpty(secondCategoryInfoList)){
                for(DubboAmpMobileTreeCategoryInfo secondCategoryInfo : secondCategoryInfoList){
                    Map<String, Object> secondMap = new HashMap<>();
                    secondMap.put("categoryId", secondCategoryInfo.getWid());
                    secondMap.put("categoryName", secondCategoryInfo.getName());
                    // 处理service
                    List<Map<String, Object>> secondServiceList = new ArrayList<>();
                    List<String> secondServiceWids = secondCategoryInfo.getRecursionServiceWids();
                    if(CollectionUtils.isNotEmpty(secondServiceWids)){
                        for(String serviceWid : secondServiceWids){
                            DubboAmpMobileServiceInfo serviceInfo = serviceMap.get(serviceWid);
                            if(null != serviceInfo && (flag || serviceInfo.getHasPermission())){
                                setAppIds.add(serviceInfo.getWid());
                                Map<String, Object> objectMap = NewMobileClientDataAdapter.toClientAppData(serviceInfo);
                                if (objectMap != null) {
                                    secondServiceList.add(objectMap);
                                }
                            }
                        }
                        if (CollectionUtils.isNotEmpty(secondServiceList)) {
                            secondMap.put("apps", secondServiceList);
                        }
                    }
                    List<Map<String, Object>> threeCategoryList = new ArrayList<>();
                    secondMap.put("children", threeCategoryList);
                    secondCategoryList.add(secondMap);

                    // 三级分类
                    List<DubboAmpMobileTreeCategoryInfo> threeCategoryInfoList = secondCategoryInfo.getChildren();
                    if(CollectionUtils.isNotEmpty(threeCategoryInfoList)){
                        for(DubboAmpMobileTreeCategoryInfo threeCategoryInfo : threeCategoryInfoList){
                            Map<String, Object> threeMap = new HashMap<>();
                            threeMap.put("categoryId", threeCategoryInfo.getWid());
                            threeMap.put("categoryName", threeCategoryInfo.getName());
                            // 处理service
                            List<Map<String, Object>> threeServiceList = new ArrayList<>();
                            List<String> threeServiceWids = threeCategoryInfo.getRecursionServiceWids();
                            if(CollectionUtils.isNotEmpty(threeServiceWids)){
                                for(String serviceWid : threeServiceWids){
                                    DubboAmpMobileServiceInfo serviceInfo = serviceMap.get(serviceWid);
                                    if(null != serviceInfo && (flag || serviceInfo.getHasPermission())){
                                        setAppIds.add(serviceInfo.getWid());
                                        Map<String, Object> objectMap = NewMobileClientDataAdapter.toClientAppData(serviceInfo);
                                        if (objectMap != null) {
                                            threeServiceList.add(objectMap);
                                        }
                                    }
                                }
                                if (CollectionUtils.isNotEmpty(threeServiceList)) {
                                    threeMap.put("apps", threeServiceList);
                                }
                            }
                            threeCategoryList.add(threeMap);
                        }
                    }
                }
            }
        ...
        //处理未分类的服务
        categoryListNew(categoryList, flag, serviceInfoList, categoryInfoList, setAppIds);
        return dataTableDataGroupByCategory(categoryList, setAppIds.stream().distinct().collect(Collectors.toList()).size());
    }

    private void categoryListNew(List<Map<String, Object>> categoryList, boolean flag, List<DubboAmpMobileServiceInfo> serviceInfoList, List<DubboAmpMobileTreeCategoryInfo> categoryInfoList, List<String> setAppIds) {
        //处理未分类的服务
        List<Map<String, Object>> unCategoryList = new ArrayList<>();
        for (DubboAmpMobileServiceInfo serviceInfo : serviceInfoList) {
            boolean find = false;
            if(setAppIds.contains(serviceInfo.getWid())){
                find = true;
            }
    ...
    public Map<String, Object> toClientAppListGroupByCategory(List<DubboAmpMobileServiceInfo> serviceInfoList, String categoryId) {
        List<Map<String, Object>> categoryList = new ArrayList<>();
        Map<String,String> systemConfigMap = ApplicationContextUtil.get(SystemConfigService.class)
                .getSystemConfigsForMap(Lists.newArrayList("isFilterAppForTodaySchool","categoryIdForTodaySchool"));
        // 读取配置 是否需要过滤重复应用 0 过滤 其他 不过滤
        String isFilter = "1";
        // 配置表里面配置的分类id
        String configCategoryId = null;
        if (MapUtils.isNotEmpty(systemConfigMap)) {
            isFilter = systemConfigMap.containsKey("isFilterAppForTodaySchool") ? systemConfigMap.get("isFilterAppForTodaySchool") : isFilter;
            configCategoryId = systemConfigMap.get("categoryIdForTodaySchool");
        }
        DubboAmpMobileCategoryResponse categoryResponse = null;
        //仅查询configCategoryId分类下的子分类（用作查询二级分类）
        if(!StringUtils.isEmpty(configCategoryId)){
            categoryResponse = serviceAmpMobileService.getCategoryAndServiceListByFirstCategoryId(configCategoryId);
        }else{
            categoryResponse = serviceAmpMobileService.getCategoryAndServiceList(categoryId);
        }

...
import com.wisedu.minos.casp.portal.service.impl.ExecutorService;
...
public class IDSLoginFilter implements Filter {
    ...
    public IDSLoginFilter (CasProperties casProperties,Long redisExpireTime,ExecutorService executorService) {
    ...
    public void doFilter (ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if ( isStaticResource(requestUri) || IGNORE_URL_LIST.contains(requestUri)
            ...
            //用户登录，向使用分析上报
            if(StringUtils.endsWithAny(requestUri, Global.LOGIN_URI)&&null!=userInfo&&StringUtils.isNotBlank(userInfo.getWid())){
                Map<String, Object> map = new HashMap<>();
                map.put("userAccount",userInfo.getWid());
                map.put("loginTime", DateUtil.getStrFromDate(new Date(),DateUtil.DATE_FORMATE_STRING_A));
                map.put("platformType", CommonUtil.isMobileDevice()?1:0);
                map.put("loginIp",CommonUtil.getRemoteAddress(httpRequest));
                executorService.sendMqMsg(MinosConstants.RabbitMqExchangeQueue.EXCHANGE_STATA_PORTAL_USER_LOGIN, JSON.toJSONString(map));
                executorService.sendMqMsg(MinosConstants.RabbitMqExchangeQueue.EXCHANGE_PORTAL_USER_LOGIN, JSON.toJSONString(map));
            }
            return;
        }
        ...
        if(userInfo!=null){
            if(null!=httpRequest.getSession().getAttribute("user")){
                currentUser= ( UserInfo ) httpRequest.getSession().getAttribute("user");
            }else{
                currentUser=ApplicationContextUtil.get(UserUtil.class).getUserByAccount(userInfo.getUserAccount());
                httpRequest.getSession().setAttribute("user", currentUser);
                httpRequest.getSession().setAttribute("minosUserInfo", currentUser);
            }
        }
        ...
        if (null == currentUser) {
            pageContextCacheKey = "pageContext:visitorKey:" + pageCode + ":" + platformType + ":" + siteWid;
        } else {
            pageContextCacheKey = "pageContext:" + currentUser.getUserAccount() + ":" + pageCode + ":" + platformType + ":" + siteWid;
        }
        ...
        if(!redisUtil.isHasKey(pageContextCacheKey)||(redisUtil.isHasKey(pageContextCacheKey)&&null==pageContextResult)){
            try {
                pageContext = ApplicationContextUtil.get(PageInfoService.class).getPageContext(httpRequest,pageCode,siteWid);
                redisUtil.set(pageContextCacheKey, pageContext, redisExpireTime, TimeUnit.SECONDS);
            } catch ( Exception e ) {
                String url = httpRequest.getRequestURL().toString();
                if ( e instanceof NoLoginException ) {
                    //未登录或者菜单不在该权限下，跳转
                    String loginPage = getLoginPage(currentUser, isNoLoginException, httpRequest, pageCode, originalUrl);
                    ...
                    chain.doFilter(request, response);
                    return;
                } else if(e instanceof PageCodeNotInShowProgramException){
                    MultiSitesAdapter multiSitesAdapter = ApplicationContextUtil.get(MultiSitesAdapter.class);
                    String redirectUrl = multiSitesAdapter.getRouteByRequest((HttpServletRequest) request) + Global.INDEX_URI;

                    logger.error("pageCode:{}不在当前展示方案",pageCode);
    ...
    private String getLoginPage(UserInfo currentUser, Boolean isNoLoginException, HttpServletRequest httpRequest,
                                String pageCode,String originalUrl) throws UnsupportedEncodingException {
        String loginPage ="";
...
//            httpRequest.getSession().setAttribute("localLoginHref", pageCode);
            httpRequest.getSession().setAttribute("localLoginHref", originalUrl);

...
import com.wisedu.minos.api.model.DubboUserDetailReq;
...
public class UserUtilImpl implements UserUtil {
    ...
    public UserInfo getUserByAccount(String userAccount) {
        DubboUserInfo userDetail = null;
        String checkIconSwitch = ApplicationContextUtil.get(SystemConfigService.class).getSystemConfigValue(Global.GET_LOGIN_USER_IMAGE_OSS);
        if(null != checkIconSwitch && "1".equals(checkIconSwitch)){
            DubboUserDetailReq dubboUserDetailReq = new DubboUserDetailReq();
            dubboUserDetailReq.setUserAccouont(userAccount);
            userDetail = userService.getUserDetailByUserAccount(dubboUserDetailReq);
        }else{
            userDetail = userService.getUserDetailByUserAccount(userAccount);
        }
        ...
        userInfo.setAllParentOrgIncludeSelf(userDetail.getAllParentOrgIncludeSelf());
        return userInfo;
    }
    ...
    @Override
    public void clearUserCache(String userAccount) {
        String partKey = "userId:" + userAccount  + ":cardId:";

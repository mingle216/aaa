...
public class CardUtilImpl implements CardUtil {
    ...
    @Override
    public List<String> queryCardConfig(String cardWid, String cardId, int platformType) {
        ...
        wrapper.lambda().eq(!StringUtils.isEmpty(cardWid), PageCardConfigEntity::getCardWid, cardWid)
                .eq(!StringUtils.isEmpty(cardId), PageCardConfigEntity::getCardId, cardId)
                .eq(PageCardConfigEntity::getPlatformType, localPlatformType.getType())
                .eq(PageCardConfigEntity::getIsDeleted, Global.DeleteStatus.NO.getId());
        //从数据库获取卡片配置
        List<String> cardStrList = Lists.newArrayList();
        List<PageCardConfigEntity> pageCardConfigEntities = pageCardConfigMapper.selectList(wrapper);
        if(!CollectionUtils.isEmpty(pageCardConfigEntities)){
            pageCardConfigEntities = pageCardConfigEntities.stream().peek(e -> {
                e.setCardConfig(filterHttpOrHttps(e.getCardConfig()));
            }).collect(Collectors.toList());

            pageCardConfigEntities.forEach(e -> {
                if ( null != e ) {
                    if (StringUtil.isEmpty(e.getCardConfig())) {
                        //当此卡片配置为空时，取卡片默认配置
                        return;
                    }

                    String str = StringEscapeUtils.unescapeJava(filterHttpOrHttps(e.getCardConfig()));
                    Map<String, Object> cardConfigMap = getCardConfigMap(cardId);
                    JSONObject jsonObject = JSONObject.parseObject(str.substring(1, str.length() - 1));
                    if( MapUtils.isNotEmpty(cardConfigMap) ){
                        Set<String> keySets = cardConfigMap.keySet();
                    ...
                    str =  "\"" + StringEscapeUtils.escapeJava(JSON.toJSONString(jsonObject, SerializerFeature.WriteMapNullValue)) + "\"";
                    cardStrList.add(str);
                }
            });
        }
    ...
    @Override
    public ComponentContainer getComContainerConfig(String cardWid, String cardId, String valueConfig, int platformType, String templateWid,String pageCode) {
        ICard iCard = getCard(cardId);
        Global.PlatformType localPlatformType = Global.PlatformType.getByType(platformType);
        String configJson = homeService.getConfigJson(cardId,localPlatformType.getType());
        return iCard.getConfig(new CardConfigReq(cardId, cardWid, configJson, new ComponentContainer(configJson, filterHttpOrHttps(valueConfig)),platformType,templateWid,pageCode));
    }
...
//        ShowProgrammeEntity defaultProgramme = showProgrammeService.getDefaultProgramme(platformType,HttpUtil.getValueFromSessionAndCookie(Global.SITE_WID,request));
        ShowProgrammeEntity defaultProgramme = getShowProgramme();
        Integer isShowPcService = defaultProgramme.getIsShowPcService();
        return 1 == isShowPcService;
    }

    @Override
    public boolean getIsPcServiceSupported() {
        ShowProgrammeEntity defaultProgramme = getShowProgramme();
        // 0 不支持 1 支持
        Integer isShowPcService = defaultProgramme.getIsPcServiceSupported();
        return 1 == isShowPcService;
    }

    public ShowProgrammeEntity getShowProgramme(){
        int platformType=CommonUtil.isMobileDevice(request)?Global.PlatformType.MOBILE.getType():Global.PlatformType.PC.getType();
        return showProgrammeService.getDefaultProgramme(platformType,HttpUtil.getValueFromSessionAndCookie(Global.SITE_WID,request));
    }
}

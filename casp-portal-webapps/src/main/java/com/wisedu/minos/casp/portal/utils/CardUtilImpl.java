...
public class CardUtilImpl implements CardUtil {
    ...
    public String filterHttpOrHttps(String data){
        if(StringUtil.isNotEmpty(data)&&data.contains("/oss/")){
            List<Integer> indexStr = CommonUtil.getIndexStr("/oss/", data);
            List<String> replaceResult = new ArrayList<>();
            for ( int i = 0 ; i <indexStr.size()  ; i++ ) {
                if(i==0){
                    String tmp = data.substring(0,indexStr.get(0)+4);
                    replaceResult.add(tmp.substring(tmp.lastIndexOf("http"),indexStr.get(0)+4));
                }else{
                    int httpFix = data.substring(indexStr.get(i - 1), indexStr.get(i)).lastIndexOf("http");
                    replaceResult.add(data.substring(indexStr.get(i - 1)+httpFix,indexStr.get(i)+4));
                }
            }
            if( !CollectionUtils.isEmpty(replaceResult) ){
                AtomicReference<String> result= new AtomicReference<>("");
                replaceResult.forEach(str->{
                    result.set(data.replaceAll(str, minosOssAddResses));
                });
                return result.get();
            }
        }

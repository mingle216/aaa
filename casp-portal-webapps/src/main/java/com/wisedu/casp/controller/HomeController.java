...
import com.wisedu.minos.casp.portal.model.site.SiteRule;
...
public class HomeController extends AbstractController {
    ...
    private static final String CONFIG_AGENT_WXWORK = "config.agent.wxwork";
    private static final String CONFIG_AGENT_DINGTALK = "config.agent.dingTalk";
    private static final String CONFIG_AGENT_WECHAT_CLIENT = "config.agent.wechat.client";
    private static final String CONFIG_AGENT_WELINK = "config.agent.weLink";
    ...
    public ModelAndView getPageView() {
        ...
        modelAndView.setView(new RedirectView(Global.INDEX_URI, true, false));
        return modelAndView;
//        return "redirect:" + Global.INDEX_URI;
    ...
    public ResultData getPageView(HttpServletRequest httpRequest) {
        ...
        int platformType = RequestUtil.getPlatform(httpRequest).getType();

        String getPageViewCacheKey = "";
        if (null == currentUser) {
            getPageViewCacheKey = "getPageView:visitorKey:" + pageCode + ":" + platformType + ":" + siteWid;
        } else {
            getPageViewCacheKey = "getPageView:" + currentUser.getUserAccount() + ":" + pageCode + ":" + platformType + ":" + siteWid;
        }
        ...
        try {
            ...
            if (!redisUtil.isHasKey(getPageViewCacheKey) || (redisUtil.isHasKey(getPageViewCacheKey) && null == getPageViewResult)) {
                ...
                TemplateContext templateContext = new TemplateContext(activeMenuId, pageContext.getPageInfoEntity(),
                        pageContext.getShowProgrammeEntity(), templateEntity, pageContext.getMenuTree(), sidebarGroup,
                        null, pageContext.getPortalSelectMenus());
            ...
            // 设置终端设备
            result.put("showMobileTitle", showMobileTitle(httpRequest));
        } catch (Exception e) {
    ...
    private boolean showMobileTitle(HttpServletRequest request) {
        if (CommonUtil.isMobileDevice()) {
            String userAgent = request.getHeader("User-Agent");
            List<SystemConfigEntity> patterns = systemConfigService.getSystemConfigs(Arrays.asList(CONFIG_AGENT_WXWORK, CONFIG_AGENT_WECHAT_CLIENT, CONFIG_AGENT_DINGTALK, CONFIG_AGENT_WELINK));
            for (SystemConfigEntity pattern : patterns) {
                String value = StringUtil.isNotEmpty(pattern.getConfigValue()) ? pattern.getConfigValue() : pattern.getDefaultValue();
                if (StringUtil.isNotEmpty(value) && userAgent.toLowerCase().matches(value)) {
                    return false;
                }
            }
        }
    ...
    public ResultData getTodoTaskCount(@RequestParam("userId") String userId) {
        try {
            return ResultData.success(homeService.getTodoTaskCount(URLDecoder.decode(userId, "utf-8")));
        } catch (Exception e) {
            logger.error("====getTodoTaskCount发生异常：{}", e);
        }
    ...
    public ResultData getUserMessageCount(@RequestParam("userId") String userId) {
        try {
            return ResultData.success(homeService.getUserMessageCount(URLDecoder.decode(userId, "utf-8")));
        } catch (Exception e) {
            logger.error("====getUserMessageCount发生异常：{}", e);
        }
        return ResultData.success("0");
    }
     ...
     * @Param serviceId -> 服务表中的wid   serviceWid -> 服务表中的serviceId
     * @return ResultData
     **/
    @RequestMapping("/serviceShow")
    @ResponseBody
    public ResultData serviceShow(HttpServletRequest request, @RequestParam(value = "serviceId", defaultValue = "") String serviceId, @RequestParam(value = "isMobile", defaultValue = "0") Integer isMobile,
                                  @RequestParam(value = "serviceWid", defaultValue = "") String serviceWid) {
        if (StringUtil.isEmpty(serviceId) && StringUtil.isEmpty(serviceWid)) {
            return ResultData.error("传参错误，请检查");
        }
        //查询服务信息
        ResultData resultData = queryServiceByWid(serviceId, "0", serviceWid, request);
        ...
        if ("0".equals(resultData.getErrcode())) {
            ...
            serviceId = dubboServiceInfo.getWid();
            serviceUrl = getServiceUrl(dubboServiceInfo.getServiceStation().intValue(), dubboServiceInfo.getMobileAccessUrl(), dubboServiceInfo.getPcAccessUrl());
    ...
    private String getServiceUrl(int serviceStation, String mobileAccessUrl, String pcAccessUrl) {
        String serviceUrl;
        if (serviceStation == 0) {
            serviceUrl = pcAccessUrl;
        } else if (serviceStation == 1) {
            serviceUrl = mobileAccessUrl;
        } else {
            serviceUrl = CommonUtil.isMobileDevice() ? mobileAccessUrl : pcAccessUrl;
        }
        return serviceUrl;
    }
    ...
    @ResponseBody
    public ResultData queryServiceByWid(@PathVariable("serviceWid") String serviceWid, @PathVariable("isFilterService") String isFilterService,
                                        String serviceId, HttpServletRequest request) {
        if (StringUtil.isEmpty(serviceWid) || "null".equals(serviceWid) || "undefined".equals(serviceWid)) {
            serviceWid = "";
        }
        DubboServiceInfoApiResp dubboServiceInfoApiResp = homeService.queryServiceByWid(serviceWid, serviceId);
        Map<String, Object> result = new HashMap<>();
    ...
    @ResponseBody
    public ResultData queryI18nList(@RequestParam("lang") String lang, HttpServletRequest request) {
        return homeService.queryI18nList(lang, request);
    }
     ...
     * @date 2022/9/6 17:02
     */
    @RequestMapping("/collectOneThing")
    @ResponseBody
    @MustLogin
    public ResultData collectOneThing(@RequestParam("id") String id, @RequestParam("operate") String operate) {
        String userId = userUtil.getUserAccount();
        String partKey = "userId:" + userId + ":cardId:";
        Set<String> keys = redisUtil.scan(partKey);
        redisUtil.deleteKeys(keys);
        ResultData resultData = homeService.collectOneThing(id, operate);
        return resultData;
    }
    ...
    @ResponseBody
    @MustLogin
    public ResultData collectTask(@RequestParam("id") String id, @RequestParam("operate") String operate) {
        String userId = userUtil.getUserAccount();
        String partKey = "userId:" + userId + ":cardId:";
        Set<String> keys = redisUtil.scan(partKey);
        redisUtil.deleteKeys(keys);
        return homeService.collectTask(id, operate);
    }
    ...
    @ResponseBody
    @MustLogin
    public ResultData collectMessage(@RequestParam("id") String id, @RequestParam("operate") String operate) {
        String userId = userUtil.getUserAccount();
        String partKey = "userId:" + userId + ":cardId:";
        Set<String> keys = redisUtil.scan(partKey);
        redisUtil.deleteKeys(keys);
        return homeService.collectMessage(id, operate);
    }
    ...
    @ResponseBody
    @MustLogin
    public ResultData collectNews(@RequestParam("id") String id, @RequestParam("operate") String operate) {
        String userId = userUtil.getUserAccount();
        String partKey = "userId:" + userId + ":cardId:";
        Set<String> keys = redisUtil.scan(partKey);
        redisUtil.deleteKeys(keys);
        return homeService.collectNews(id, operate);
    }
    ...
    public ResultData execCardMethod(HttpServletRequest httpRequest, @RequestBody CardAjaxRequest cardAjaxRequest, @PathVariable("cardId") String cardId
        ...
        if (!homeService.checkCardMethodAuth(cardId, cardWid)) {
            throw new BusinessException("无权限调用");
        }
    ...
    public ResultData getCardConfig(@RequestParam("cardId") String cardId, @RequestParam("cardWid") String cardWid
        ...
        String valueConfig = "";
        try {
            PageCardConfigEntity pageCardConfigEntity = cardUtil.getCardConfigByCardWidFromDb(cardWid, String.valueOf(platformType));
            // 权限校验 （cardWid -> pageWid -> programmeId）
            String pageWid = pageCardConfigEntity.getPageWid();
            PageInfoEntity pageInfo = pageInfoService.getById(pageWid);
            if (null == pageInfo) {
                return ResultData.error("该卡片没有对应的页面信息");
            }
            MinosConsoleUtil.checkSiteAuth(Collections.singletonList(pageInfo.getProgrammeId()));
            valueConfig = pageCardConfigEntity.getCardConfig();
        } catch (MinosException e) {
            logger.debug("获取卡片配置异常【cardId】==" + cardId + "==【cardWid】==" + cardWid, e);
            return ResultData.error(e.getMessage());
        } catch (Exception e) {
            logger.debug("卡片【cardId】==" + cardId + "==【cardWid】==" + cardWid + ",当前未有保存新配置，开启使用当前默认配置configMetaData.json", e);
        }
        return ResultData.success(cardUtil.getComContainerConfig(cardWid, cardId, valueConfig, platformType, templateWid, pageCode));
    }
    ...
    public ResultData getTemplateConfig(HttpServletRequest request, @RequestParam("templateId") String templateId
        ...
        // 权限校验
        MinosConsoleUtil.isRequestRightsLegal(Collections.singletonList(showProgramme.getSiteWid()));
     ...
     **/
    @IgnoreGatewaySecurity
    @ManagerGatewayApi(name = "获取或保存展示方案对应的本地化样式代码", realPath = "/common/getProgrammeLocalStyle", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
...
//                }
//            }
            redisUtil.save(key, (proLocalStyle == null || null == proLocalStyle.getProgrammeLocalStyle()) ? "" : proLocalStyle.getProgrammeLocalStyle());
        }
        return ResultData.success(JSON.parseObject(redisUtil.get(key).toString(), String.class));
    }

    @ManagerGatewayApi(name = "获取或保存展示方案对应的本地化样式代码", realPath = "/common/getOrSaveProgrammeLocalStyle", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
    @PostMapping(value = {"/common/getOrSaveProgrammeLocalStyle"})
    @ConsoleRightCheck(roleType = MenuEditTypeEnum.MENU_EDIT_TYPE_SET, menu = MenuEditTypeEnum.MENU_ID_PROGRAMME_MANAGE)
    @ResponseBody
    public ResultData getOrSaveProgrammeLocalStyle(@RequestBody ProgrammeLocalStyleReq programmeLocalStyleReq) {
        if (StringUtil.isEmpty(programmeLocalStyleReq.getWid())) {
            return ResultData.error("传参错误，请检查");
        }

        // 权限校验
        MinosConsoleUtil.checkSiteAuth(Collections.singletonList(programmeLocalStyleReq.getWid()));
     ...
     * @return ResultData
     **/
    @ManagerGatewayApi(name = "门户获取或切换黑白模式", realPath = "/programme/switchBlackAndWhiteMode", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
     ...
     * @return
     */
    @ManagerGatewayApi(name = "门户获取或切换移动端黑白模式", realPath = "/programme/switchMobileBlackAndWhiteMode", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
    ...
    public ResultData getPortalConfig(@RequestBody PortalConfigReq portalConfigReq) {
        if (StringUtil.isNotEmpty(portalConfigReq.getKey())) {
            return ResultData.success(systemConfigService.getSystemConfigForPortal(portalConfigReq.getKey()));
        }
     ...
     * @return ResultData
     **/
    @ManagerGatewayApi(name = "保存门户配置(多个)", realPath = "/base/savePortalConfigMultiple", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
    ...
    public ResultData savePortalConfigMultiple(@RequestBody PortalConfigMultipleReq portalConfigMultipleReq) {
        ...
        if (CollectionUtils.isNotEmpty(portalConfigList)) {
            for (PortalConfigReq portalConfigReq : portalConfigList) {
                String errMsg = verifyParams(portalConfigReq);
    ...
    /***
     * @Author jcx
     * @Description 保存用户首次切换站点记录
     * @Date 14:03 2022/5/12
     * @Param body:
     * @return void
     **/
    @RequestMapping("/saveSiteitching")
    @ResponseBody
    @MustLogin
    public void saveSiteWitching(@RequestBody SaveSitewitchingReq body) {
        homeService.saveSiteWitching(body);
    }
     ...
     * @return ResultData
     **/
    @RequestMapping("/queryUserSiteWitching")
    @ResponseBody
    @MustLogin
    public ResultData queryUserSitewitching(@RequestBody SaveSitewitchingReq body) {
        return ResultData.success(homeService.queryUserSiteWitching(body));
    }
     ...
     * @return ResultData
     **/
    @RequestMapping("/delUserSite")
    @ResponseBody
    @MustLogin
    public ResultData delUserSite(@RequestBody SaveSitewitchingReq body) {
        homeService.delUserSite(body);
        return ResultData.success();
    }

    @ManagerGatewayApi(name = "站点切换规则", realPath = "/site/siteSwitchRule", operationType = ApiOperationTypeEnums.API_OPERATION_QUERY)
    @OperationLog(id = "20001", name = "站点切换规则", operateObject = "站点切换规则")
    @RequestMapping("/site/siteSwitchRule")
    @ResponseBody
    public ResultData<String> querySiteSwitchRule() {
        String val = systemConfigService.getSystemConfigValue("SITE_RULE");
        return ResultData.success(val);
    }

    @ManagerGatewayApi(name = "站点规则保存", realPath = "/site/updateSiteSwitchRule", operationType = ApiOperationTypeEnums.API_OPERATION_MODIFY)
    @OperationLog(id = "20001", name = "站点规则保存", operateObject = "站点规则保存")
    @RequestMapping("/site/updateSiteSwitchRule")
    @ResponseBody
    public ResultData updateSiteSwitchRule(@RequestBody SiteRule siteRule) {
        systemConfigService.update(new UpdateWrapper<SystemConfigEntity>().lambda()
                .eq(SystemConfigEntity::getConfigKey, "SITE_RULE").set(SystemConfigEntity::getConfigValue, siteRule.getCode()));
        return ResultData.success();
    }
     ...
     * @Author jcx
     * @Description PC端事项跳转
     * @Date 15:20 2022/8/29
     * @return ModelAndView
     **/
    @GetMapping("/simJump")
    public ModelAndView simJump(HttpServletRequest request, @RequestParam("id") String id, @RequestParam("name") String name,
                                @RequestParam("isOnline") int isOnline, @RequestParam("langCountry") String langCountry) {
        ...
        templateAjaxRequest.setCurrentUser(userUtil.getCurrentUser());
        Map<String, String> itemVisitParam = Maps.newHashMap();
        itemVisitParam.put("id", id);
        itemVisitParam.put("type", Global.VisitType.ITEM.getType());
        Map<String, String> serviceVisitParam = Maps.newHashMap();
        serviceVisitParam.put("id", id);
        serviceVisitParam.put("type", Global.VisitType.SERVICE.getType());
        dubboCaspItemJumpReq.setItemWid(id);
        dubboCaspItemJumpReq.setLang(langCountry);
        ...
        if ("0".equals(result.getErrcode()) && null != result.getData()) {
            DubboCaspItemJump dubboCaspItemJump = result.getData();
            name=dubboCaspItemJump.getItemName();
            if (1 == dubboCaspItemJump.getIsEnabled()) {
                if (1 == dubboCaspItemJump.getIsShow() && 0 == isOnline) {
                    //直接跳转事项详情
                    //记录访问事项
                    executorService.visit(itemVisitParam, templateAjaxRequest);
                    modelAndView.setView(new RedirectView(siteRoute + Global.INDEX_URI + "#/itemDetail?wid=" + id + "&&name=" + URLEncoder.encode(name), true, false));
                    return modelAndView;
                }
                List<String> linkServiceWids = dubboCaspItemJump.getLinkServiceWids();
                if (CollectionUtils.isNotEmpty(linkServiceWids)) {
                    UserInfo currentUser = userUtil.getCurrentUser();
                    ServiceByUserAndWidsReq req = new ServiceByUserAndWidsReq();
                    req.setUserWid(null != currentUser ? currentUser.getWid() : "");
                    req.setData(linkServiceWids);
                    req.setHandlerServiceFavorite(false);
                    //查询服务
                    ...
                    if ("0".equals(dubboServiceRes.getErrcode()) && CollectionUtils.isNotEmpty(dubboServiceRes.getData())) {
                        List<DubboServiceData> data = dubboServiceRes.getData();
                        List<ServiceRelServiceResponse.ServiceModel> serviceModels = ObjectUtil.copyListProperties(data, ServiceRelServiceResponse.ServiceModel::new);
                        List<ServiceRelServiceResponse.ServiceModel> serviceListByPlatform = serviceItemApiService.getServiceListByPlatform(serviceModels, linkServiceWids);
                        if (CollectionUtils.isNotEmpty(serviceListByPlatform)) {
                            if (1 == serviceListByPlatform.size()) {
                                //该事项下只有一个服务
                                ServiceRelServiceResponse.ServiceModel serviceModel = serviceListByPlatform.get(0);
                                if ((0 == serviceModel.getServiceStation() || 2 == serviceModel.getServiceStation()) &&
                                        1 == serviceModel.getPermission()) {
                                    //PC端服务且有权限
                                    ResultData resultData = serviceShow(request, serviceModel.getWid(), 0, serviceModel.getServiceId());
                                    if ("0".equals(resultData.getErrcode()) && null != resultData.getData()) {
                                        Map<String, Object> serviceData = (Map<String, Object>) resultData.getData();
                                        if (serviceData.containsKey("grantData")) {
                                            List<ServiceGrant> grantData = (List<ServiceGrant>) serviceData.get("grantData");
                                            if (CollectionUtils.isNotEmpty(grantData)) {
                                                if (1 == grantData.size()) {
                                                    //记录访问服务
                                                    serviceVisitParam.put("id", serviceModel.getWid());
                                                    executorService.visit(serviceVisitParam, templateAjaxRequest);
                                                    modelAndView.setView(new RedirectView(grantData.get(0).getServiceUrl(), true, false));
                                                    return modelAndView;
                                                }
                                            }
                                        } else {
                                            ...
                                            serviceVisitParam.put("id", serviceModel.getWid());
                                            executorService.visit(serviceVisitParam, templateAjaxRequest);
                                            modelAndView.setView(new RedirectView(String.valueOf(serviceData.get("serviceUrl")), true, false));
                                            return modelAndView;
                                        }
                                    }
                                    modelAndView.setView(new RedirectView(siteRoute + Global.INDEX_URI + "#/ServiceShow?isMobile=0&&wid=" + serviceModel.getWid(), true, false));
                                    return modelAndView;
                                }
                            }
                        }
        ...
        //跳转到中间页判断，老逻辑
        modelAndView.setView(new RedirectView(siteRoute + Global.INDEX_URI + "#/chooseService?wid=" + id + "&&name=" + URLEncoder.encode(name) + "&isOnline=" + isOnline, true, false));
        return modelAndView;
    }

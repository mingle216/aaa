...
public class HomeController extends AbstractController {
    ...
    private final List<String> viewMethods = Arrays.asList(new String[]{"render", "renderData", "view", "viewData"
            ,"getMarkNumber", "getConfig","getSubscribedChannels","getChannelNews","getConfiguredChannel","getPersonalDataDetail","getPersonalDataList"});
    ...
    public ResultData getPageView(HttpServletRequest httpRequest) {
        ...
        if(null==currentUser){
            getPageViewCacheKey="getPageView:visitorKey:"+pageCode;
        }else{
            getPageViewCacheKey="getPageView:"+currentUser.getUserAccount()+":"+pageCode;
        }
            ...
            if(!redisUtil.isHasKey(getPageViewCacheKey)||(redisUtil.isHasKey(getPageViewCacheKey)&&null==getPageViewResult)){
                ...
                if(Global.PlatformType.MOBILE.getType()==templateContext.getShowProgrammeEntity().getPlatformType()){
                    ...
                    if(activeMenu==null||!Global.ROOT_PAGE_ID.equals(templateContext.getPageInfoEntity().getParentId())){
                        result.put("isShowBackButton",true);
                    }else{
                        result.put("isShowBackButton",false);
                    }
                }
                // URL参数
    ...
    @ResponseBody
    public ResultData getSidebarCount(@RequestParam("wid") String wid){
        return ResultData.success(homeService.getSidebarCount(wid));
    }
    ...
    @ResponseBody
    public ResultData serviceShow(HttpServletRequest request,@RequestParam("serviceId") String serviceId,@RequestParam("serviceUrl") String serviceUrl){
        if(StringUtil.isEmpty(serviceId)||StringUtil.isEmpty(serviceUrl)){
        ...
        if("0".equals(resultData.getErrcode())){
            ...
            DubboServiceInfo dubboServiceInfo = dubboServiceInfos.get(0);
    ...
    public ResultData execCardMethod(HttpServletRequest httpRequest, @RequestBody CardAjaxRequest cardAjaxRequest,@PathVariable("cardId") String cardId
        ...
        String key = "userId:" + userUtil.getUserAccount() + ":cardId:" + cardId + ":cardWid:" + cardWid + ":method:";
        String paramKey ;
        if (viewMethods.contains(method)) {
            key = key + method+ ":param:"+requestParam;;
            result = redisUtil.get(key);
            paramKey = key + ":param:"+requestParam;
            if (null == result || !redisUtil.isHasKey(paramKey)) {
                result = executeCardMethod(cardAjaxRequest);
                logger.debug("key"+ key + "存缓存");
                redisUtil.set(key, result, redisExpireTime, TimeUnit.SECONDS);
                redisUtil.set(paramKey, requestParam, redisExpireTime, TimeUnit.SECONDS);
            }else {
                if (!redisUtil.get(paramKey).equals(requestParam)){
    ...
    public ResultData getCardConfig(@RequestParam("cardId") String cardId,@RequestParam("cardWid")String cardWid
        ...
        logger.info("getCardConfig【cardId】==" +cardId+"==【cardWid】=="+cardWid );
        String valueConfig="";
        try {
            valueConfig=cardUtil.getCardConfigByCardWidFromDb(cardWid,String.valueOf(platformType)).getCardConfig();
        } catch ( Exception e ) {
            logger.error("卡片【cardId】=="+cardId+"==【cardWid】=="+cardWid +",当前未有保存新配置，开启使用当前默认配置configMetaData.json" , e);
        }
    ...
    public ResultData getTemplateConfig(HttpServletRequest request,@RequestParam("templateId") String templateId
        ...
        logger.info("getCardConfig【templateId】==" +templateId+"==【templateWid】=="+templateWid );
        ...
        return ResultData.success(template.getConfig(new TemplateConfigRequest(templateId,templateWid,configJson,new ComponentContainer(configJson,valueTemplateConfig))));
    }
    ...
    public ResultData switchBlackAndWhiteMode(@RequestBody BlackAndWhiteModeReq blackAndWhiteModeReq){
        ...
        SystemConfigEntity local_blackAndWhite_mode = systemConfigService.getSystemConfig("local_blackAndWhite_Mode");
        if(!"2".equals(blackAndWhiteModeReq.getIsBlackMode())){
            //切换黑白模式
            local_blackAndWhite_mode.setConfigValue(blackAndWhiteModeReq.getIsBlackMode());
            boolean flag = systemConfigService.saveOrUpdate(local_blackAndWhite_mode);
            if(flag){
                return ResultData.success(blackAndWhiteModeReq.getIsBlackMode());
            }
        }
        return ResultData.success(StringUtils.isEmpty(local_blackAndWhite_mode.getConfigValue()) ? local_blackAndWhite_mode.getDefaultValue() : local_blackAndWhite_mode.getConfigValue());
    }
    ...
    private Object executeTemplateMethod(HttpServletRequest httpServletRequest, TemplateAjaxRequest request) {
        ...
        String templatePackageId = showProgrammeService.getDefaultProgramme(platformType.getType()).getTemplateId();

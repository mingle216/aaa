<template>
  <div class="search__bar" v-if="showSearch">
    ...
    <div class="pos-relative">
      <div class="search__btn__wrap" :class="[isFocus?'select-focus':'']">
        <div class="search-tip_box" v-show="!keyword && !isFocus">
          <i class="el-icon-search search__btn-icon"></i>
          <span>{{placeholderEllipsis}}</span>
        </div>
        <we-input  ref="InputKeyword" size="large" v-model="keyword" :placeholder="newplaceholder" :maxlength="100" @focus="handleFocus"
          @blur="handleBlur" @input="handleInput" @keyup.native="handleKeyUp" >
            <i slot="prefix" v-show="keyword || isFocus" class="el-input__icon el-icon-search el-icon-search_input"></i>
        </we-input>
        <!-- <i class="clear-icon we-icon-circle-close" v-if="keyword" @click="clearKeyword"></i> -->
        <i class="iconfont icon-biaoqianguanbi clear-icon" v-if="keyword" @click="clearKeyword"></i>
        <!-- <div
          class="search__btn portal-primary-color-lv1 ellipsis"
          @click="handleSearch"
        >
          {{ $Lan("SYS_TEMPLATE_WORK", "search", "搜索") }}
        </div> -->
      ...
      <div class="search__dropdown portal-font-color-lv1" v-show="dropdownShow"
        ...
        <vue-scroll>
          <div class="search__dropdown__lists">
            ...
            <associative-dropdown v-else ref="AssociateDown" :keyword="searchKeyword" :searchTab="selectedTab">
...
export default {
  ...
  data () {
    return {
      ...
      tabLists: [
        {
          groupWid: "all",
          groupName: this.$Lan("CUS_TEMPLATE_CQUPT", "all", "全部"),
        },
      ],
  ...
  computed: {
    ...
    pageConfig () {
      console.log('pageRenderData',this.pageRenderData)
    ...
    // 未登录或者无历史数据时，不展示历史记录
    showHistory () {
      console.log('showHistory',this.pageConfig,(
        this.showSearch &&
    ...
    newplaceholder() {
      if(!this.keyword && this.isFocus){
          return  this.placeholderEllipsis
      }else{
        return ""
      }
    }
    ...
    handleFocus () {
      ...
      this.dropdownShow =
        this.dropdownType === "HistoryDropdown" ? this.showHistory : true;
          console.log('dddddd',this.dropdownShow,this.dropdownType)
      ...
      window.shell.execTemplateMethod(
        ...
        () => {
          ...
          const placeholder = this.placeholder.replace(reg, (match) =>
            ...
            let url =`search?searchKey=${keyword}&groupWid=${this.selectedTab.groupWid}&groupType=${this.selectedTab.groupType}&placeholder=${placeholder}`
            ...
            data.forEach((element) => {
              if (element.groupType != "1") {
                ...
                element.groupName = item
                  ? this.$Lan("CUS_TEMPLATE_CQUPT", item.key, item.value)
                  : element.groupName;
              }
            });

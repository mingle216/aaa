<template>
  <div class="login__wrap">
    <!-- 导航栏配置 -->
    <nav-link v-if="headerNavBar.length" style="margin-right: 10px;" :headerNavBar="headerNavBar" />
    <!-- 消息 -->
    <div class="we-dropdown-link mr-10" v-if="currentUser && messageUrl" @click="goToMessageCenter">
      <we-tooltip class="item" effect="dark" :content='$Lan("CUS_TEMPLATE_CQUPT", "message", "消息")' placement="bottom" :open-delay="100">

        <div class="c-box">
          <we-badge :hidden="ishasUnReadMsgUrl == '1' || count == 0" :value="count" :max="99">
    ...
    <!-- 切换站点 -->
    <we-dropdown v-if="siteList.length > 1" @command="handleCommandSites" placement="bottom" class="mr-10" trigger="click">
      <we-tooltip class="item" effect="dark" :content='$Lan("CUS_TEMPLATE_CQUPT", "site", "站点")' placement="bottom" :open-delay="100">
       <div class="c-box">  <span class="icon iconfont icon-fangan we-dropdown-link"> </span>
       </div>
      </we-tooltip>
      <we-dropdown-menu slot="dropdown">
        <we-dropdown-item :command="item.siteRoute"
          class="portal-font-backgroundcolor-hover-lv5 portal-primary-color-hover-lv1 drop__site__item"
          v-for="item in siteList" :key="item.wid">
          <span class="text__ellipsis" :title="item.siteName">
            {{ item.siteName }}
          </span>
          <i class="we-icon-check portal-primary-color-lv1" style="font-size:14px;font-weight:bold;margin-right:0"
            v-if="item.wid === currentSite.wid" />
        </we-dropdown-item>
      </we-dropdown-menu>
    </we-dropdown>
    <!-- 多语言 -->
    <language-menu :pageRenderData="pageRenderData" :currentUser="currentUser" class="mr-10" />
    <!-- 登陆用户菜单 -->
     <we-dropdown v-if="currentUser" @command="handleCommand" trigger="click">
        <we-tooltip class="item" effect="dark" :content='$Lan("CUS_TEMPLATE_CQUPT", "user", "用户")' placement="bottom" :open-delay="100">
       <div class="c-box">  <span class="we-dropdown-link">
      ...
      <we-dropdown-menu slot="dropdown" class="ty_drop_menu" style="padding: 0px !important; margin: 4px 0px 0px 0px !important;">
        ...
        >
          <span class="logout__item text__ellipsis">
            {{ $Lan("SYS_TEMPLATE_WORK", "logout1", "退出") }}
          </span>
        </we-dropdown-item>
      </we-dropdown-menu>
    </we-dropdown>
    <div v-else class="login__btn_box portal-primary-backgroundcolor-hover-lv5">
      <div class="login__btn portal-primary-color-lv1" @click="handleLogin">
        {{ $Lan("SYS_TEMPLATE_WORK", "login", "请登录") }}
      </div>
      ...
      <we-dropdown v-if="portalSelectMenus && portalSelectMenus.length > 0" @command="handleCommand">
        <div class="a_bot">
          <i class="we-icon-caret-bottom portal-primary-color-lv1"></i>
        </div>
        <we-dropdown-menu slot="dropdown" class="ty_drop_menu" style="padding: 0px !important; margin: 4px 0px 0px 0px !important;">
...
.l_line{
  width: 1px; height: 30px; background-color: rgb(217, 217, 217);
}
.login__btn {
  width: 66px;
  height: 30px;

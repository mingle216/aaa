<template>
  <div v-loading="loading" ref="recommendapp">
    <AutoContainer :con-type="containerType.type" :con-height="containerType.value" :scroll="{ scrollingX: true }"
      v-if="apps && apps.length">
      <div class="recommendapp">
        <ul :style="{ width }">
          <li v-for="(item, index) in apps" :key="`${item.serviceName}-${index}`" v-serviceLayout="{ columns }">
            <div class="recommendapp-list" :class="{
              ...
              <div class="recommendapp-left" :class="{ 'no-permission-service': !item.permission }">
                <img :src="item.iconLink" alt="加载失败" @error="
                  () => {
              ...
              <div class="recommendapp-center">
                <we-tooltip class="item" effect="dark" :content="item.serviceName" placement="top-start"
                  :open-delay="800">
              ...
              <div class="recommendapp-right" v-if="userInfo">
                <i class="iconfont icon-favorites" :class="{
                  favorite_font: item.favorite,
                  unfavorite_font: !item.favorite,
                }" @click.stop="collectApp(item)"></i>
              </div>
              <div v-if="userInfo && serviceAppraise == '1'" class="app-tag portal-primary-backgroundcolor-lv1"
                @click.stop="openServiceComment(item)">
                <i class="we-icon-edit" />
              </div>
            </div>
          </li>
          <li @click="goAllservice(allServiceUrl)" v-show="isShowAllService !== 2" v-serviceLayout="{ columns }">
            <div
              ...
              <div class="recommendapp-center">
                <h3 class="ellipsis">
                  {{ $Lan("SYS_CARD_RECOMMENDAPP", "allServices", "全部服务") }}
                </h3>
              </div>
            </div>
          </li>
    ...
    <EmptyCon :tip="$Lan('SYS_CARD_RECOMMENDAPP', 'noData', '暂无相关服务')" :height="containerType.value"
      v-if="!apps || !apps.length"></EmptyCon>
  </div>
</template>

...
export default {
  ...
  methods: {
      ...
      window.shell.execCardMethod(
        ...
        res => {
          ...
          if (res.errcode === '0' && res.data) {
            // const cc  = res.data.filterItem || []
            //  this.getfwxn = cc.filter(item => item.alias == "服务主题")[0].data.filter((item) => {return item.subjectName == '数字驾驶舱'})[0].subjectWid;
            //   this.searchServiceItemParam1.categoryWids= this.getfwxn
              // console.log(,'this.searchServiceItemParam1');
            //  this.searchData1()
            // 组装筛选条件
    ...
    searchData1() {
      console.log(this.searchServiceItemParam1,'this.searchServiceItemParam1');
      this.loading = true
      window.shell.execCardMethod(
        {
          ...this.card,
    ...
    setData(data) {
      ...
      if(this.serviceItemList){
         const aaa = this.serviceItemList.map((item) => {
          return item.datas.filter(ele => {
            return !(ele.itemCategory.indexOf('数字驾驶舱') > -1);
          })
        })
       this.serviceItemList.forEach((element, index) => { 
          element.datas = aaa[index]
       })
      }

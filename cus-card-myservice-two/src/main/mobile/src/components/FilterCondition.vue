...
export default {
  ...
  mounted(){
     this.getsubjectname= this.filterItem[0].data.filter(item => {
      console.log(!(item.subjectName.indexOf('数字驾驶舱')>-1),'iiiiii');
      return !(item.subjectName.indexOf('数字驾驶舱')>-1)
     })
    //  console.log(this.getsubjectname,'getsubjectname');
  }
}
</script>

      ...
      if (isInit) {
        ...
        if (!this.hasInited && this.configInfo.categoryWid) {
          ...
          this.treeData.forEach(el => {
            if (el.categoryWid === this.configInfo.categoryWid) {
              arr.push(el)
            }
            if (el.children && el.children.length) {
              const temp = el.children.find(d => d.categoryWid === this.configInfo.categoryWid)
              temp && arr.push(temp) && (el.childSelect = true) && (el.expanded = true)
            }
          })
          curr = arr[0];
          if (!curr) {

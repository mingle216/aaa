  ...
  >
    <div class="dialog-body">
      <div class="serve-info">
        ...
        <we-tooltip  class="serve-name ellipsis portal-font-color-lv1"
                          effect="dark"
      ...
      <div class="dialog-tip portal-font-color-lv1">
        {{ $Lan($TEMPLATE_NAME, 'dialogNewpage', '此服务需跳转新页面打开，是否打开？') }}
      </div>
    </div>
    <span slot="footer" class="dialog-footer">
      <we-button class="btn-cancel portal-primary-border-color-hover-lv1 portal-primary-color-hover-lv1" @click="dialogVisible = false">{{ $Lan($TEMPLATE_NAME, 'cancel', '取 消') }}</we-button>
      <we-button class="btn-commit  portal-primary-backgroundcolor-lv1" type="primary" @click="goLink">{{ $Lan($TEMPLATE_NAME, 'confirm', '确 定') }}</we-button>
    </span>
  </we-dialog>
</template>

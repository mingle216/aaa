<template>
  <div class="newsTab-wrap">
      ...
      <div class="newsTab__left portal-font-color-lv2" ref="NewsTab">
        ...
        >
          <div
            ...
            :key="item.wid"
            :class="[
              item.wid === selectedTab ? 'active portal-primary-backgroundcolor-lv1' : 'portal-primary-backgroundcolor-hover-lv4 portal-primary-color-hover-lv1',
            ]"
    ...
    <!-- 订阅modal -->
    <we-dialog
      :title="$Lan('CUS_CARD_MYNEWS', 'programLists', '栏目列表')"
      :append-to-body="true"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <we-button
          ...
          @click="handleClose"
          >{{ $Lan("CUS_CARD_MYNEWS", "cancel", "取 消") }}</we-button
        >
        <we-button
          type="primary"
          :class="[
          ...
          :disabled="disabled"
          @click="handleOk"
          >{{ $Lan("CUS_CARD_MYNEWS", "ok", "确 定") }}</we-button
        >
      </div>
    </we-dialog>
  </div>
    ...
    handleUpdateNews() {
      this.$emit(
        ...
        this.selectedTab === "all"
          ...
          : this.selectedTab,this.selectedTab === "all"?'全部':this.tabName
      );
    },

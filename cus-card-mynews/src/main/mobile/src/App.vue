    ...
    >
          ...
          >
            <div class="newsAnnouncement-listFather">
              ...
              >
                ...
                >
                  <span class="ellipsis">{{
                    $Lan("CUS_CARD_MYNEWS_h5", "showMore", "查看更多")
                  }}</span>
                  <i
      ...
      <EmptyCon v-else :height="179">
        <we-button
          ...
          {{
            $Lan("CUS_CARD_MYNEWS_h5", "goSubscribe", "点击订阅")
          }}</we-button
        >
        <p class="portal-font-color-lv2">
          {{ $Lan("CUS_CARD_MYNEWS_h5", "noChannel", "暂无栏目") }}
        </p>
      </EmptyCon>
    </div>
    <subscribe
  ...
  methods: {
    ...
    filterTime(time) {
      ...
      let TimeArr = [
        {
          time: 15 * 60,
          text: this.$Lan("CUS_CARD_MYNEWS_h5", "timeJust", "刚刚"),
        },
        {
          time: 60 * 60,
          text: (time) =>
    ...
    setAll() {
      if (this.tabs.length > 1) {
        this.tabs.unshift({
          name: this.$Lan("CUS_CARD_MYNEWS_h5", "allChannle", "全部"),
          wid: this.tabs
            .map((v) => {
              return v.wid;
            })
